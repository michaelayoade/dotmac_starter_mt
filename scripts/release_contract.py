#!/usr/bin/env python3
"""Resolve, inspect and verify a product-owned STATELESS CONTRACT CATALOGUE.

The fail-closed half of `release-contract.yml`, and the sibling of
`scripts/release_module.py`, `scripts/release_adapter.py` and
`scripts/release_connector.py`.

THIS LANE CARRIES ITS OWN EXTRACTION CLASSIFICATION, and that is a reversal.
An earlier draft of this file asserted the opposite — "a release profile, not a
new extraction classification" — by analogy with the connector ruling, which
correctly refused to mint `connector-plugin` because the four properties
`stateless-protocol-adapter` governs (no `ModuleManifest`, no lineage, no
`MIGRATION_OWNER_LEDGER` allocation, no persistence import) are exactly a
connector's four. That ruling stands for connectors and is superseded here,
because those four properties are silent on the property that separates these
two shapes: NETWORK REACH.

An adapter EXISTS to reach a provider — `dotmac-auth-oidc` ships `transport.py`
and declares `httpx` in its own reviewed `allowed_requires`, and ADR-0006 names
network I/O one of the two concerns that cannot be faked. A catalogue reaches
nothing at all: it is held data, canonical schema bytes and the digests that
attest them, so `FORBIDDEN_IMPORT_ROOTS` below refuses `httpx`, `requests`,
`aiohttp`, `urllib`, `urllib3`, `socket` and `subprocess` outright. One word
covering both shapes
would have to PERMIT the network import, because the adapter genuinely needs
it — and a catalogue that acquired a provider client would then be caught by
nothing except whether somebody remembered to route it through the stricter
lane. So `stateless-contract-catalogue` buys an enforceable property rather
than a synonym (ADR-0006's 2026-08-26 amendment).

The allowlist is intentionally closed and currently EMPTY: absence is the
publication lock, not this file. Every subcommand refuses rather than warns;
publishing a malformed ownership contract would let every later plan be
...
#: The DECLARED `EXTRACTION.toml` classification this lane publishes. Checked,
#: never trusted: `resolve` reads the package's own dossier and refuses anything
#: else, so a package whose facts belong in the module, adapter or connector
#: lane cannot be published by the lane that would ask it the fewest questions.
CLASSIFICATION: Final = "stateless-contract-catalogue"
...
#: Facts that belong to another lane. Their PRESENCE is the refusal: a package
#: carrying a schema, a manifest attribute, a control-plane floor or a connector
#: registration has questions this lane does not know how to ask.
...
# A SUPERSET of ADR-0006's fifth property, deliberately. The classification
# checker (`contract_catalogue_violations` in
# `tests/architecture/test_product_first_extraction.py`) names the seven
# network roots the ADR states; this lane forbids those PLUS the persistence
# and remote-execution roots, because a package reaching the registry has
# cleared more gates than one merely claiming the classification. The seven
# must all appear here — a lane laxer than the classification it publishes
# would make the classification the weaker of the two checks, which is the
# wrong way round.
...
def load_policy() -> dict:
    ...
    if conformance.get("classification") != CLASSIFICATION:
        raise ReleaseRefused(
            f"{ALLOWLIST.name}: conformance.classification must be "
            f"{CLASSIFICATION!r}; this lane publishes exactly one classification"
        )
    for key in ("package_dir_prefix", "package_dir_suffix", "required_exports"):
        if key not in conformance:
            raise ReleaseRefused(f"{ALLOWLIST.name}: conformance is missing {key}")
    return data
...
def cmd_resolve(args: argparse.Namespace) -> None:
    ...
    # `kernel_floor` is emitted because this is the ONE place the floor is read
    # from the allowlist and proved to have a release tag. The verify job pins
    # it on the registry install rather than spelling it a second time in a
    # shell step, where it could drift from the number that was gated.
    for key in ("package_dir", "import_name", "owner_code", "tag_prefix"):
        print(f"{key}={entry[key]}")
    print(f"kernel_floor={entry['kernel_floor']}")
    dependency_dirs = [
...
def cmd_verify_registry(args: argparse.Namespace) -> None:
    ...
    entry = resolve(distribution)
    # An explicit floor pin, or pip resolves the catalogue's own `>=<floor>`
    # against the index and picks the NEWEST kernel — which would certify the
    # same grammar as the unpinned leg and never execute the declared floor at
    # all. The floor may only be the one `resolve` gated and proved tagged.
    pins = [args.pin]
    if args.kernel_floor:
        if args.kernel_floor != entry["kernel_floor"]:
            raise ReleaseRefused(
                f"{distribution}: --kernel-floor {args.kernel_floor!r} is not the "
                f"gated floor {entry['kernel_floor']!r}"
            )
        pins.append(f"dotmac-kernel=={args.kernel_floor}")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _verify_installed(python, entry, version)
    print(f"registry verification OK for {' '.join(pins)}")
...
def main() -> int:
    ...
    registry_parser.add_argument("--kernel-floor", default="")
