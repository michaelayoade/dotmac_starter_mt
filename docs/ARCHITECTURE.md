...
packages/dotmac-digital-media/   optional tenant Digital Asset Management owner
  pyproject.toml                 distribution dotmac-digital-media; audit-complete,
  EXTRACTION.toml                with Mkt as the narrow reusable-asset source and
                                 Backoffice as the first candidate cutover
  src/dotmac_digital_media/      stable asset identity, immutable source
                 revisions, descriptive metadata and extractor observations,
                 collections/classification, immutable rights versions,
                 exact-source repairable renditions, media grants/review,
                 usage observations and append-only evidence; manifest plus
                 independent `dm` lineage in schema `mod_digitalmedia`.
                 Tenant plane only with forced RLS. BUILT AND TESTED HERE, NOT
                 COMPOSED by this reference assembly. Files owns bytes;
                 provider adapters perform extraction/transformation; Content,
                 Documents and Publishing own relationship meaning; Records
                 owns retention/hold/disposition only after exact-revision
                 declaration (ADR-0033).
...
| `MediaLibrary` / `MediaCollection` / `MediaCollectionItem` / `MediaClassificationAssignment` / `MediaSavedSelection` | `mod_digitalmedia.media_libraries`, `media_collections`, `media_collection_items`, `media_classification_assignments`, `media_saved_selections` | `dotmac-digital-media` optional module | Tenant DAM navigation, ordering and controlled organization. Folders/collections/classification never become asset identity; campaign/product/event associations are opaque consumer-owned references (ADR-0033; [`digital-media-sources.md`](inventories/digital-media-sources.md)). |
| `MediaAsset` / `MediaRevision` / `MediaRelationship` | `mod_digitalmedia.media_assets`, `media_revisions`, `media_relationships` | `dotmac-digital-media` optional module | Stable reusable media identity with a tenant-composite canonical pointer and immutable numbered source revisions. Each revision stores an opaque Files UUID/checksum/media type/length; replacement never rewrites history. Revision relationships cover derivation, parent and alternate-language evidence without importing a sibling owner. |
| `MediaMetadataObservation` / `MediaRightsVersion` | `mod_digitalmedia.media_metadata_observations`, `media_rights_versions` | `dotmac-digital-media` optional module | Append-only exact-checksum extractor observations and immutable rights/licence/territory/channel/purpose/window versions. Provider extractors and legal/privacy validity remain supplied evidence; Digital Media owns deterministic enforcement, not their decisions. |
| `MediaRendition` | `mod_digitalmedia.media_renditions` | `dotmac-digital-media` optional module | Repairable projection binding one exact source revision/checksum to a recipe/version, engine/version and opaque output Files UUID/checksum. A rendition never becomes the source revision implicitly. |
| `MediaAccessGrant` / `MediaAnnotation` / `MediaUsageObservation` / `MediaEvent` | `mod_digitalmedia.media_access_grants`, `media_annotations`, `media_usage_observations`, `media_events` | `dotmac-digital-media` optional module | Library/collection/asset grants, exact-revision review, deduplicated consumer-owned usage references and append-only lifecycle/access/rights/transformation evidence. Usage observations answer who uses a revision but do not transfer relationship or Records authority. |
...
| Reusable media identity, revisions, rights, renditions and media evidence | `dotmac_digital_media.service` is the sole writer: `create_asset`, concurrency-safe `add_revision`, descriptive/technical observation writes, collection/classification/relationship writes, immutable `set_rights`, lifecycle-aware `evaluate_usage`, exact `apply_rights_deadline`, `request_rendition`/`request_rendition_regeneration`/completion/failure, lifecycle transition, media grants/annotations, superseding usage observations and `evaluate_disposition`. Access derives the asset's real library/collections rather than trusting caller scope. Every path receives and flushes the caller session. Files owns bytes and scanning; Approvals supplies exact-content verdicts; Durable Timers owns wake-up mechanics; Content/Documents/Publishing own relationship meaning; Records alone authorizes retention/hold/disposition after declaration. The package stores only opaque sibling refs and imports no sibling/provider (ADR-0033). |
