...
packages/dotmac-documents/       optional tenant controlled-document owner
  pyproject.toml                 distribution dotmac-documents; audit-complete,
  EXTRACTION.toml                product-first from ERP HR handbooks with Sub's
                                 immutable quote-export behavior as a named delta
  src/dotmac_documents/          stable libraries/types/documents, immutable
                 content versions and renditions, lifecycle/access/checkouts,
                 annotations/acknowledgements and append-only evidence; its
                 independent `dc` lineage owns schema `mod_documents`.
                 BUILT HERE, NOT COMPOSED by this reference
                 assembly. Files owns bytes, Approvals owns verdicts and
                 Durable Timers owns scheduled wake-up mechanics (ADR-0033).
packages/dotmac-records/         optional tenant managed-records owner
  pyproject.toml                 distribution dotmac-records; audit-complete,
  EXTRACTION.toml                greenfield-after-inventory because no audited
                                 product contains a qualifying Records authority
  src/dotmac_records/            immutable file-plan/schedule versions, exact
                 declarations, trigger observations, holds, disposition,
                 preservation/custody and append-only evidence; its independent
                 `re` lineage owns schema `mod_records`. BUILT HERE,
                 NOT COMPOSED by this reference assembly. It takes authority
                 only after explicit declaration and never owns source-domain
                 business meaning or Files physical state (ADR-0033).
...
| `DocumentLibrary` / `DocumentTypeVersion` / `Document` | `mod_documents.document_libraries`, `document_type_versions`, `documents` | `dotmac-documents` optional module | Product-first from ERP's stable HR handbook code/type organization, corrected into separate stable identity and immutable policy versions. Folder is navigation only; the current-version pointer is database-constrained to a version of the same document (ADR-0033; [`documents-records-sources.md`](inventories/documents-records-sources.md)). |
| `DocumentVersion` / `DocumentRendition` | `mod_documents.document_versions`, `document_renditions` | `dotmac-documents` optional module | ERP checksum/version/acknowledgement base plus Sub's immutable snapshot and content-addressed replay behavior. Both rows bind opaque Files identity and checksums; renderer/OCR provenance is an observation, never source content or billing-renderer authority. |
| `DocumentClassification` / `DocumentRelation` | `mod_documents.document_classifications`, `document_relations` | `dotmac-documents` optional module | Native controlled-document taxonomy assignments and typed internal relationships. Neither becomes source-domain business identity. |
| `DocumentCheckout` / `DocumentAnnotation` | `mod_documents.document_checkouts`, `document_annotations` | `dotmac-documents` optional module | Native exclusive reservations and exact-version review findings. General conversation remains with Inbox. |
| `DocumentAccessGrant` / `DocumentAcknowledgement` / `DocumentEvent` | `mod_documents.document_access_grants`, `document_acknowledgements`, `document_events` | `dotmac-documents` optional module | Native document-scoped access/deny inheritance, exact-version read attestation and append-only evidence. Kernel authentication/global permission remains prerequisite; Documents owns only object-specific access. |
| `RetentionScheduleVersion` / `RecordSeriesVersion` | `mod_records.retention_schedule_versions`, `record_series_versions` | `dotmac-records` optional module | Native after the fleet audit found no qualifying implementation. Immutable schedule/file-plan definitions pin triggers, cutoff, action, approval policy, authority and versioned hierarchy (ADR-0033; [`documents-records-sources.md`](inventories/documents-records-sources.md)). |
| `Record` / `RecordTriggerObservation` | `mod_records.records`, `record_trigger_observations` | `dotmac-records` optional module | Exact source/file/series/schedule declaration snapshot plus deduplicated authoritative-domain observations. Records owns retention consequences after declaration but never invoice/employee/contract/ticket meaning. |
| `LegalHoldCase` / `LegalHoldTarget` | `mod_records.legal_hold_cases`, `legal_hold_targets` | `dotmac-records` optional module | Native simultaneous holds over exact records, pinned series or snapshotted cohorts. Every active hold independently blocks disposition; releasing one never releases another. |
| `DispositionBatch` / `DispositionItem` | `mod_records.disposition_batches`, `disposition_items` | `dotmac-records` optional module | Immutable batch membership and digest-bound, four-eye-capable disposition evidence. Records conditionally rechecks holds, schedule and source observation before authorizing Files; it records destruction only after matching Files confirmation. |
| `CustodyTransfer` / `PreservationCheck` / `RecordEvent` | `mod_records.custody_transfers`, `preservation_checks`, `record_events` | `dotmac-records` optional module | Native custody manifests, fixity/physical-state observations and append-only evidence. Provider location is an observation, not provider authority; search remains a rebuildable projection. |
...
| Controlled document identity, exact versions and document-scoped lifecycle | `dotmac_documents.service` is the sole writer after a product cutover: `create_library`, `create_document_type_version`, `create_document`, `add_version`, `transition_document`, `schedule_document_transition`, `add_rendition`, classification/relations, checkout, annotation, access and acknowledgement paths. Version allocation locks the stable document and advances one same-document current pointer; future transitions emit typed Durable Timer requests and bind the exact source version. Files, Approvals and Durable Timers remain independent owners of bytes, verdicts and wake-up mechanics (ADR-0033). |
| Declared-record retention, holds, preservation and disposition | `dotmac_records.service` is the sole compliance writer after explicit declaration: immutable schedule/series publication, `declare_record`, typed trigger observation, hold placement/release, disposition evaluation/batching/approval/authorization/confirmation, preservation checks and custody transfers. The source domain retains business meaning and supplies typed state observations; Records must authorize deletion and Files must confirm the resulting physical state. Undeclared attachments remain with their source domain (ADR-0033 and amended ADR-0022). |
