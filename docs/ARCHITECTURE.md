...
becoming a connector concern (ADR-0032):
...
| Origin verification and rate limiting for a collection request | the adopting application's thin local/remote collection adapter | Both local and Integrator-delivered commands enter the same versioned analytics service; remote transport is not a bypass |
| Remote delivery/replay evidence | `dotmac-integration` inside `dotmac_integrator` plus a website connector plugin | Transports a typed observation and records delivery; never classifies, sessionises or attributes it |
| Website routes/content and form submission | `dotmac-sites` / `dotmac-forms` in the adopting product | May emit a declared anonymous event after its own decision; analytics receives no form/customer fields |
...
| Consent eligibility and official acquisition/commercial consequences | product/kernel consent owner; future attribution owner; Sales/Orders/Billing | Analytics records the effective policy evidence and anonymous sequence only |
