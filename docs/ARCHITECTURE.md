...
| Flutterwave API v4 exact-byte HMAC-SHA256 ingress authentication, provider-event identity, exact amount/currency translation and acknowledgement bytes | `dotmac-connector-flutterwave` (`flutterwave`, INGRESS-only, `payments.settlement.observation.v1`) | Owns no v3 fallback, provider HTTP call, provider-fee inference, installation row, retry/checkpoint, destination, tenant, allocation, coverage, receivable, ledger or product consequence |
| Paystack ingress authentication, provider-event identity, exact amount/fee/currency translation and acknowledgement bytes | `dotmac-connector-paystack` (`paystack`, INGRESS-only, `payments.settlement.observation.v1`) | Owns no provider HTTP call, installation row, retry/checkpoint, destination, tenant, allocation, coverage, receivable, ledger or product consequence |
...
plane as their first candidate assembly. AI Operations, Remote Access and
Compliance Reporting now have manifests, independent lineages, catalogue
entries and a shared live tenant-isolation canary in this branch; this is
construction, not composition, publication or adoption. The other accepted
owners enter the as-built registry only with the same evidence.
