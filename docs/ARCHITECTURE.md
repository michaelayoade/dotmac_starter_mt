...
packages/dotmac-procurement/     optional tenant procurement decision owner
  pyproject.toml                 distribution dotmac-procurement; audit-complete,
  EXTRACTION.toml                with ERP as the qualifying source and first
                                 authority cutover; Backoffice then Sub are
                                 only candidate consumers
  src/dotmac_procurement/        typed requisition, sourcing, immutable offer,
                 evaluation/award, purchase-commitment and receipt-observation
                 lifecycle; manifest plus independent `pc` lineage in schema
                 `mod_procurement`. All twelve tables use forced tenant RLS.
                 BUILT AND TESTED HERE, NOT COMPOSED by this reference
                 assembly. Supplier identity, budgets, approvals, Inventory,
                 Assets, Payables, product work and delivery stay external.
...
| `PurchaseRequisition` / `PurchaseRequisitionLine` | `mod_procurement.purchase_requisitions`, `purchase_requisition_lines` | `dotmac-procurement` optional module | Tenant-local purchase demand and its immutable submitted line snapshot. Budget and approval are digest-bound facts from their owners; requester, item, cost-centre and product subject identities remain opaque references (ADR-0038; [`procurement-sources.md`](inventories/procurement-sources.md)). |
| `SourcingEvent` / `SourcingEventLine` / `SourcingInvitation` | `mod_procurement.sourcing_events`, `sourcing_event_lines`, `sourcing_invitations` | `dotmac-procurement` optional module | Direct/selective/open-competitive sourcing decision with an aware publication window, exact criteria totalling 100 and a database-protected line/invitation snapshot. Supplier identity and statutory threshold policy remain outside the owner. |
| `BidSubmission` / `BidLine` / `BidEvaluation` | `mod_procurement.bid_submissions`, `bid_lines`, `bid_evaluations` | `dotmac-procurement` optional module | One immutable formally received offer per supplier and source identity, exact criterion coverage, selected in-event bid and digest-bound approval of the award. Product project/vendor bidding remains with Sub until a separate adapter cutover. |
| `PurchaseOrder` / `PurchaseOrderLine` / `ReceiptObservationRecord` / `ProcurementEvidence` | `mod_procurement.purchase_orders`, `purchase_order_lines`, `receipt_observations`, `procurement_evidence` | `dotmac-procurement` optional module | Exact buyer commitment and append-only receipt/transition evidence. Line receipt quantities are a rebuildable fulfilment projection with over-receipt refusal; Inventory owns stock and receipts, while Payables owns invoices, matching, payment, tax and journals. |
...
| Procurement requisition, sourcing, award and purchase commitment | `dotmac_procurement.service` — requisition create/revise/submit plus digest-bound budget/approval/rejection facts; exact requisition-line sourcing, invite/publish/receive/close/cancel/evaluate/award; one source-bound purchase-order commitment plus batch-atomic deduplicated receipt observations and cancel/close. Every path takes explicit `TenantScope`, receives and flushes the caller session, and writes append-only evidence in the same transaction. Approvals decides actor/quorum policy, Inventory supplies receipt facts and owns stock, and Payables consumes `ApprovedPurchaseFact`; none writes Procurement lifecycle state directly (ADR-0038). |
