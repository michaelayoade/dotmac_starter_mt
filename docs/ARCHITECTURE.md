...
- `dotmac-release-catalog` associates a typed module snapshot or complete
  product snapshot with exact artifact bytes as the distinct, singular
  `module_database_catalog` or `product_database_catalog` attestation; and
...
an empty or fictitious persistence identity, while Release Catalog joins both
claims to the same immutable artifact when both exist.
