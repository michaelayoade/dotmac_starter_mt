...
| `AccountCategory` / `Account` | `mod_accounting.account_categories`, `accounts` | `dotmac-accounting` optional module | Product-first port of ERP's tenant chart. Categories classify accounts; only active posting accounts admit journal lines. No supplier, customer, asset, tax or product ORM reference crosses into the chart (ADR-0041; [`accounting-payables-sources.md`](inventories/accounting-payables-sources.md)). ERP remains live authority until the sealed first-adopter cutover. |
| `FiscalYear` / `FiscalPeriod` / `PeriodEvent` | `mod_accounting.fiscal_years`, `fiscal_periods`, `period_events` | `dotmac-accounting` optional module | Non-overlapping fiscal intervals and explicit open/soft-close/reopen/irreversible-lock transitions. Close evidence is append-only; checks are typed observations supplied by their domain owners, not re-decided by Accounting. |
| `AccountingDimension` / `AccountingDimensionValue` | `mod_accounting.accounting_dimensions`, `accounting_dimension_values` | `dotmac-accounting` optional module | Open tenant dimension vocabulary. Products add rows rather than schema columns; a journal line admits at most one value per dimension and posted evidence snapshots both codes. |
| `JournalEntry` / `JournalLine` / `JournalLineDimension` | `mod_accounting.journal_entries`, `journal_lines`, `journal_line_dimensions` | `dotmac-accounting` optional module | The sole balanced-posting and reversal decision aggregate. Producers submit source-versioned facts; they never write journal or ledger rows. Posted content is immutable and corrected only by a linked opposite journal. |
| `PostedLedgerLine` / `PostedLedgerDimension` | `mod_accounting.posted_ledger_lines`, `posted_ledger_dimensions` | `dotmac-accounting` optional module | Append-only general-ledger evidence written atomically with posting. Account, source and dimension codes are immutable snapshots; the rows are evidence, not a mutable balance cache. |
| `SupplierInvoice` / `SupplierInvoiceLine` | `mod_payables.supplier_invoices`, `supplier_invoice_lines` | `dotmac-payables` optional module | Product-first port of ERP AP's supplier-document lifecycle and liability inputs. Supplier, procurement, receipt, tax and account identities are bounded opaque evidence; submission freezes commercial facts and approval recognizes the liability (ADR-0042). ERP remains live authority until Accounting then Payables complete sealed cutovers. |
| `CreditNote` / `CreditNoteLine` / `CreditApplication` | `mod_payables.credit_notes`, `credit_note_lines`, `credit_applications` | `dotmac-payables` optional module | Supplier-credit lifecycle and append-only applications. Approval recognizes credit once; applying it only burns down available credit and an obligation projection, never creates a second economic event. |
| `PaymentObligation` / `LiabilityEvent` | `mod_payables.payment_obligations`, `liability_events` | `dotmac-payables` optional module | What is owed, in which currency and by when, plus append-only liability evidence. These rows are neither bank/payment instructions nor Accounting journal rows. |
| `SettlementObservation` / `AccountingReceipt` | `mod_payables.settlement_observations`, `accounting_receipts` | `dotmac-payables` optional module | Immutable, source-versioned observations from the Treasury/payment owner and from an assembly-delivered Accounting consequence. Payables imports neither owner and performs no provider I/O or journal write. |
...
| Accounting chart, periods, dimensions, journals and immutable posted evidence | `dotmac_accounting.service` — chart/dimension/fiscal creation, explicit period transitions, `create_journal`, `post_journal` and `reverse_journal`. Kernel idempotency owns replay; database triggers protect posted and period evidence. Producers provide typed opaque facts and cannot write either journal or ledger tables. This is the candidate shared owner under ADR-0041; ERP remains the production writer until its shadow reconciliation and sealed cutover retire the source paths. |
| Supplier documents, credits, liabilities and payment obligations | `dotmac_payables.service` — invoice/credit creation, submission, approval and void-before-approval; credit application; settlement observation; consequence building and accounting-receipt recording. Procurement, supplier identity, approvals, Accounting and Treasury remain separate owners. This is the candidate shared owner under ADR-0042; ERP remains the production writer until Accounting cuts over first and the later Payables cutover retires every source AP writer. |
