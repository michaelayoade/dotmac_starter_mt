...
packages/dotmac-inventory/       tenant stock ledger and traceability (`iv`)
packages/dotmac-assets/          tenant durable-unit lifecycle owner (`as`)
packages/dotmac-ipam/            tenant address-space owner (`ip`)
packages/dotmac-network-inventory/    tenant network-device identity (`ni`)
packages/dotmac-network-observability/ tenant network observation evidence (`no`)
packages/dotmac-network-topology/     tenant forwarding topology owner (`nt`)
packages/dotmac-network-assurance/    tenant maintenance/assurance owner (`na`)
packages/dotmac-network-control/      tenant control-plane intent owner (`nc`)
packages/dotmac-fiber-plant/     tenant outside-plant record owner (`fp`)
packages/dotmac-network-access/  tenant access-path owner (`nac`)
packages/dotmac-pon-access/      tenant PON/ONT lifecycle owner (`pn`)
...
| `Item` / `Warehouse` | `mod_inventory.items`, `warehouses` | `dotmac-inventory` optional module | Tenant-local SKU and stock-facility identity. Product source, supplier, procurement, GL, physical asset and network meaning stay outside (ADR-0041; [`inventory-sources.md`](inventories/inventory-sources.md)). |
...
| `AddressSpace` / `Pool` / `Address` / `Assignment` / `UtilizationSnapshot` / `IpamEvent` | `mod_ipam.*` | `dotmac-ipam` optional module | Address-space, pool, reservation and assignment decisions plus immutable utilization/lifecycle evidence. Subscriber/service identity is an opaque subject reference (ADR-0043). |
...
| Collections state rows | `dotmac_collections.service` is the sole persistent writer through `CollectionPolicyService`, `CollectionCaseService`, `PaymentArrangementService`, `CollectionGraceService`, `CollectionNoticeService`, `CollectionActionService` and `CollectionReconciliationService`. Every operation receives a caller-owned `Session` and explicit `TenantScope`, mutates and flushes without commit/rollback, uses the kernel idempotency ledger, serializes first-writer races, and appends request/receipt/reconciliation evidence. Case assessment rereads the typed authoritative `ReceivablesReader` and fails closed on unavailable, incomplete, non-authoritative, stale or fingerprint-conflicting positions; it never stores or calculates a second balance. The package remains adoption-gated: Sub still needs its timer/reader/action adapters, backfill, shadow parity, coupled Billing watermark and retirement ratchets before any authority switch. |
...
| Inventory items, warehouses, movements, balances, reservations, lots/serials and valuation evidence | `dotmac_inventory.service` is the only writer. Its commands lock affected projections, append immutable evidence, rebuild drift from movements/reservations, and flush without commit/rollback. A product performs the explicit issue handoff; Assets and every network module are forbidden from maintaining stock or value in parallel (ADR-0041/0038). |
| Durable assets, assignments, maintenance, disposal and lifecycle evidence | `dotmac_assets.service` is the only writer. Expected-state aggregate commands lock source state, preserve the custody chain, append immutable evidence and flush without transaction authority. Network Inventory keeps only an opaque asset correlation and network-specific state (ADR-0042/0038). |
