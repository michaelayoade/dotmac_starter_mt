...
| `ExpenseCategory` / `ExpensePolicy` / `ExpensePolicyRule` / `ExpensePolicyEvaluation` | `mod_expenses.expense_categories`, `expense_policies`, `expense_policy_rules`, `expense_policy_evaluations` | `dotmac-expenses` optional module | Tenant category rules, effective-dated immutable policy revisions and append-only evaluation evidence, product-first from ERP's live expense owner. Applicability facts are caller-resolved keys rather than People vocabulary, and published revisions cannot be rewritten (ADR-0040; [`expenses-sources.md`](inventories/expenses-sources.md)). |
| `ExpenseRequest` / `ExpenseRequestLine` | `mod_expenses.expense_requests`, `expense_request_lines` | `dotmac-expenses` optional module | Intended-spend request lifecycle and immutable submitted line snapshots, combining CRM's request contract and Sub's field-request behavior without copying work-order or project state. An approved request may seed exactly one separate claim; it is never proof that spend occurred. |
| `ExpenseClaim` / `ExpenseClaimLine` / `ExpenseReceipt` / `ExpenseLifecycleEvent` | `mod_expenses.expense_claims`, `expense_claim_lines`, `expense_receipts`, `expense_lifecycle_events` | `dotmac-expenses` optional module | Incurred-expense facts, claimed/approved amounts, domain metadata around an opaque stored-file UUID, and append-only transition evidence. Approvals supplies an opaque decision; Files owns bytes; Finance owns AP, settlement and payment. Reimbursement eligibility is derived and no mutable `paid`/`eligible` column exists (ADR-0040). |
...
| Expense requests, claims, receipts, policy decisions and reimbursement eligibility | `dotmac_expenses.service` — category and immutable policy-revision setup; request/claim creation, complete draft-snapshot revision and guarded submission; receipt attachment/verification; applying opaque Approvals decisions; request-to-claim conversion; cancellation, rejection revision and approval withdrawal; append-only evaluation/lifecycle evidence; and `reimbursement_eligibility` as a computed Finance handoff. Every path requires `TenantScope`, receives the caller session and only flushes. Party owns identity, Approvals owns quorum, Files owns bytes and Finance alone owns payable/payment state (ADR-0040). |
