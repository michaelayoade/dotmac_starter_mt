...
| `Pipeline` | `mod_sales.pipelines` | `dotmac-sales` optional module | Tenant-scoped opportunity configuration, product-first from Sub. It owns no product catalogue, fulfillment or campaign decision (ADR-0033; [`sales-sources.md`](inventories/sales-sources.md)). |
| `PipelineStage` | `mod_sales.pipeline_stages` | `dotmac-sales` optional module | Ordered tenant stage within one Pipeline, protected by same-tenant composite identity and forced RLS. |
| `Lead` | `mod_sales.leads` | `dotmac-sales` optional module | Commercial intent and its lifecycle through won/lost. Product Party/subscriber identity is an opaque typed subject snapshot supplied by the adopter, never a cross-module FK. |
| `LeadOrigin` | `mod_sales.lead_origins` | `dotmac-sales` optional module | Append-only acquisition evidence with a stable source identity. Provider, campaign and Inbox ownership remain outside sales and are not inferred from the evidence. |
| `Quote` | `mod_sales.quotes` | `dotmac-sales` optional module | Exact-money commercial offer through acceptance. Acceptance freezes the row and its canonical snapshot; SalesOrder, project, service and invoice consequences are outside this lineage. |
| `QuoteLine` | `mod_sales.quote_lines` | `dotmac-sales` optional module | Exact-money line snapshot owned with its Quote; database triggers reject every mutation once the parent Quote is accepted. |
| `QuoteDiscountRevision` | `mod_sales.quote_discount_revisions` | `dotmac-sales` optional module | Append-only discount command evidence owned with its Quote and frozen at acceptance. |
...
| Sales Pipelines, Stages, Leads, origin evidence, Quotes, lines and discount revisions | `dotmac_sales.service` is the sole owner through `create_pipeline`, `create_stage`, `create_lead`, `transition_lead`, `capture_lead_origin`, `author_quote`, `update_quote`, `change_quote_discount`, `transition_quote` and `accept_quote`. Services receive a session and flush only. Acceptance uses the kernel idempotency ledger and transactional outbox to publish one versioned `AcceptedQuoteHandoffV1`, then database triggers make the accepted Quote and its children immutable. The module stops at that output: no SalesOrder, project, work, subscription, invoice, campaign, delivery transport or product identity row is created (ADR-0033). |
