...
packages/dotmac-work-orders/     optional tenant physical-execution owner
  pyproject.toml                 distribution dotmac-work-orders;
  EXTRACTION.toml                audit-complete, Sub is first candidate adopter
  src/dotmac_work_orders/        persistence-free lifecycle, typed commands,
                 tenant models/service, manifest and independent `wo` lineage
                 in schema `mod_workorders`. BUILT AND TESTED HERE, NOT
                 COMPOSED by this reference assembly. Owns work-order status,
                 assignment history, execution events, work logs, notes and
                 evidence references. Products own subject links, workforce,
                 dispatch, inventory, topology, file bytes and consequences
                 (ADR-0031; `docs/inventories/work-orders-sources.md`).
...
| `WorkOrder` | `mod_workorders.work_orders` | `dotmac-work-orders` optional module | Tenant-scoped physical execution root, product-first from Sub's authoritative native WorkOrder path. Carries generic status, schedule, current-assignment projection and snapshotted evidence policy; subject links stay in the adopter (ADR-0031; [`work-orders-sources.md`](inventories/work-orders-sources.md)). |
| `WorkOrderAssignment` | `mod_workorders.work_order_assignments` | `dotmac-work-orders` optional module | Append-style assignment history with one database-constrained active row per work order. Assignee identity/kind are opaque product inputs; the module does not own a workforce roster. |
| `WorkOrderEvent` | `mod_workorders.work_order_events` | `dotmac-work-orders` optional module | Execution fact with a tenant-unique client event id, previous/new status, actor and optional capture coordinates. `dotmac_kernel.idempotency` owns command replay and the exact request fingerprint; this table is evidence, not a second ledger. |
| `WorkOrderWorkLog` | `mod_workorders.work_order_worklogs` | `dotmac-work-orders` optional module | Execution time fact; one open timer per tenant/actor is database-enforced so one work order cannot close another's time. |
| `WorkOrderNote` | `mod_workorders.work_order_notes` | `dotmac-work-orders` optional module | Internal/external narrative attached to one local work order. Attachments are evidence references or product-owned relations, never inline bytes. |
| `WorkOrderEvidence` | `mod_workorders.work_order_evidence` | `dotmac-work-orders` optional module | Generic evidence metadata and opaque artifact reference used by the snapshotted completion contract. File bytes and product-specific fiber/topology proof remain with their owners. |
...
| Work-order execution rows | `dotmac_work_orders.service`: its create/header-update, assign/unassign, execution-event, work-log, note and evidence commands are the only writers for the six `mod_workorders` table families. They flush without commit/rollback and delegate fingerprint-exact replay to `dotmac_kernel.idempotency`, the fleet's one at-most-once owner. Header updates cannot write lifecycle or assignment fields. The adopter validates and links subscriber/project/ticket, workforce, topology, inventory and downstream consequences without writing execution state directly. No reference-assembly surface composes the module yet (ADR-0031). |
