...
| `CoverageGate` | `mod_orders.coverage_gates` | `dotmac-orders` optional module | Orders' one-way fulfillment-eligibility decision over a finite set of opaque requirements. Sales owns the accepted membership; each external owner supplies an explicitly addressed observation receipt. `evaluate_fulfillment_eligibility` is the askable, reasoned decision owner. Billing a1 publishes no coverage fact and need not add one; an assembly may not infer evidence from receivable positions, balances or allocation arithmetic. A deferred consistency trigger proves counts and satisfaction from the append-only rows. |
...
| `Quote` / `QuoteLine` / `QuoteDiscountRevision` | `mod_sales.quotes`, `quote_lines`, `quote_discount_revisions` | `dotmac-sales` optional module | Quote authoring and immutable accepted-Quote evidence. `AcceptedQuoteHandoffV1` is the terminal boundary and carries explicit minor units, accepted price/terms/specification provenance, component taxes and finite eligibility requirement membership; Orders owns the later eligibility decision and every order consequence. |
...
| Sales pipelines, leads and Quotes | `dotmac_sales.service` owns create/transition/update commands and `accept_quote`; acceptance freezes the Quote and publishes one typed `AcceptedQuoteHandoffV1`, including the finite eligibility requirement membership accepted by the customer. Orders and every downstream commercial owner remain separate. |
...
| Customer orders, fulfillment eligibility and fulfillment-request identity | `dotmac_orders.service`: `submit_order` is the only aggregate/snapshot constructor; `accept_order` and `cancel_order` own lifecycle transitions; `record_coverage_resolution` is the only addressed evidence writer; pure `evaluate_fulfillment_eligibility` owns the reasoned decision over the frozen set; `acknowledge_fulfillment` records downstream acceptance; `reconcile_fulfillment_publications` repairs only missing/dead outbox projection from frozen authoritative inputs. `get_order_snapshot` exposes the decision and its missing requirements; `get_order_timeline` exposes the official trail. All commands take an explicit `TenantScope`, use the kernel at-most-once owner, flush without commit/rollback, and import no Billing, Subscription, Fulfillment or product implementation. The package is not composed here and owns no adapter until Sub's separate shadow/cutover change. |
