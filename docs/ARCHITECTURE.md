...
| `MetricCatalogEntry` | `mod_analytics.metric_catalog_entries` | `dotmac-analytics` optional module | Immutable tenant-local snapshot of one owner-namespaced metric declaration/version. Product-first from ERP's metric vocabulary with bounded kind/unit/granularity/dimensions replacing implicit strings and free JSON; FORCE RLS (ADR-0036; [`analytics-sources.md`](inventories/analytics-sources.md)). |
| `MetricIngestReceipt` | `mod_analytics.metric_ingest_receipts` | `dotmac-analytics` optional module | Append-only domain evidence for one accepted aggregate batch. `dotmac_kernel.idempotency` owns fingerprint comparison, replay and the concurrent-key race; exact replay returns this receipt's id from the kernel result. The receipt is never a second replay ledger, the source domain's fact or a connector checkpoint. |
| `MetricObservation` | `mod_analytics.metric_observations` | `dotmac-analytics` optional module | Append-only exact `NUMERIC(38,12)` aggregate fact linked to its receipt and declaration. It carries no subject identity, raw event payload, arbitrary SQL or domain-table FK. |
| `MetricPoint` | `mod_analytics.metric_points` | `dotmac-analytics` optional module | The sole mutable analytical projection. A deterministic `(observed_at, received_at, observation_id)` rank selects a winning observation for each declared coordinate; the complete table is rebuildable from immutable observations and never authoritative for metric meaning. |
| `MetricProjectionRebuild` | `mod_analytics.metric_projection_rebuilds` | `dotmac-analytics` optional module | Append-only before/after digest and actor/time evidence for a deterministic tenant projection rebuild. |
...
| Aggregate metric evidence and current-point projection | `dotmac_analytics.service.record_batch` is the only normal writer of declaration snapshots, source receipts, observations and winning points; `rebuild_projection` is the repair owner and rewrites only `metric_points` from immutable observations while recording digest evidence. Both accept and flush the caller's session without commit/rollback. Metric calculations remain with their declared domain owner, and adapters submit typed aggregates rather than granting analytics access to domain tables (ADR-0036). |
