...
| `AssetBook` | `mod_finance.asset_books` | `dotmac-finance` optional module | Product-first fixed-asset accounting book from ERP, corrected to keep physical asset identity/lifecycle in Assets and chart/fiscal/journal ownership outside the module. Asset, account and cost-centre links are opaque; balances are functional-currency Decimal values (ADR-0038; [`finance-asset-accounting-sources.md`](inventories/finance-asset-accounting-sources.md)). |
| `DepreciationRun` / `DepreciationLine` | `mod_finance.depreciation_runs`, `depreciation_lines` | `dotmac-finance` optional module | Calculated immutable snapshots with prospective implemented-method calculations, stale-version refusal and creator/poster separation. Posting updates books and creates one balanced consequence in the caller transaction; it never changes physical asset state. |
| `AccountingEvent` / `AccountingConsequence` / `AccountingConsequenceLine` | `mod_finance.accounting_events`, `accounting_consequences`, `accounting_consequence_lines` | `dotmac-finance` optional module | Append-only source-versioned subledger evidence for capitalization, depreciation, impairment/reversal, revaluation and derecognition. Database triggers refuse rewrites and unbalanced consequences; journal projection remains a separate local GL reconciler. |
...
| Fixed-asset accounting books and consequences | `dotmac_finance.service` — `capitalize_asset_book`, calculate/post depreciation, impairment/reversal, revaluation and derecognition. Every path receives and flushes the caller session. `dotmac-finance` owns valuation decisions and immutable balanced subledger consequences; Assets owns physical lifecycle, and product adapters own chart/fiscal/journal projection (ADR-0038). |
