...
**not** yet implement runtime profile/provider selection or the complete
effective-capability availability lifecycle. The repository now builds the
uncomposed `dotmac-subscriptions` package and its selectable tenant/platform
lineage; this reference assembly owns no commercial rows and no adopter cutover
is claimed. Billing, collections, durable timers and orders remain independent
peer owners joined only by consuming assemblies. The target architecture is authoritative in
...
component surface contains the reuse-proven `empty_state` and audit-complete
`catalog_grid`; every emitted `.dmui-*` class is derived from its typed
component contract. `catalog_grid` was sourced from Workspace application
discovery and Academy course discovery, accepts only display values, and has no
product cutover yet. Membership, authorization, price formatting, eligibility,
stock, state and actions remain caller decisions. ADR-0006 § 5 still forbids
...
| Catalog discovery card/grid | `dotmac_ui.components.catalog_grid` owns display-only inert markup and token-native component CSS; the caller supplies `CatalogItem` values. Workspace and Academy remain the local renderer owners until their later coordinated cutovers, so no independent contract consumer or retirement is claimed on this branch. |
