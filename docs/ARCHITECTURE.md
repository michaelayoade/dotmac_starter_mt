...
| Declared aggregate metric evidence and rebuildable projections | `dotmac-analytics` (ADR-0040; tenant-only reusable module, zero consumers) | Each source domain owns metric meaning and calculation; Web Analytics owns web-event/session/funnel evidence; Integrator owns transport; Analytics accepts only typed aggregate facts and owns no dashboard, report delivery, provider media or domain lifecycle |
...
packages/dotmac-analytics/       tenant aggregate evidence/projections (`ay`)
...
| `AuditEvent` | `audit_events` | core | dotmac_sub (`app/models/audit.py`, tenant-adapted); published to installable tenant modules as the verified `tenant_audit_log.v1` prerequisite, with canonical actor/forensic columns, FORCEd RLS, and append-only online grants |
...
| `Order` | `mod_orders.orders` | `dotmac-orders` optional module | Tenant-scoped stable order identity and refused-by-default lifecycle, product-first from Sub with ERP's transition/FX evidence and the mandatory immutable-snapshot delta in [`orders-sources.md`](inventories/orders-sources.md). Header money and source/FX evidence freeze in the submission transaction; a deferred database check refuses a half-snapshot or totals that differ from its lines. The separate `dotmac-sales` owner correctly stops at accepted Quote, but its current V1 handoff lacks several immutable fields needed for mechanical order creation; Sales→Orders adoption remains gated until that owner output is complete. |
| `OrderLineSnapshot` | `mod_orders.order_line_snapshots` | `dotmac-orders` optional module | One append-only accepted commercial line carrying exact value plus immutable price, terms, tax, specification and source-version evidence. Database triggers refuse update/delete and late inserts after the order freezes. |
| `CoverageGate` | `mod_orders.coverage_gates` | `dotmac-orders` optional module | Orders' one-way readiness decision over a finite set of opaque obligations. It owns no receivable, allocation or settlement decision; an explicitly contracted owner supplies typed observation receipts. Pending Billing a1 publishes no such fact, so Sub adoption is blocked until Billing supplies an owner-decided resolution or another owner is named; an assembly may not infer it from positions/arithmetic. A deferred consistency trigger proves counts and satisfaction from the append-only rows. |
| `CoverageObligation` | `mod_orders.coverage_obligations` | `dotmac-orders` optional module | One immutable member of the order's finite coverage set; a late member is structurally refused after binding. The reference is opaque and has no cross-owner foreign key. |
...
| `DomainService` | `mod_domains.domain_services` | `dotmac-domains` optional module | Tenant-only registered-domain aggregate and the one Dotmac lifecycle writer. It carries opaque order-line and offer-version references plus the commercial renewal date supplied by their owners; it never imports those owners, stores a provider identity, or writes the platform `TenantDomain` routing catalogue (ADR-0030; [`domains-sources.md`](inventories/domains-sources.md)). |
| `DomainCommand` / `DomainCommandOutcome` | `mod_domains.domain_commands`, `domain_command_outcomes` | `dotmac-domains` optional module | Append-only command intent and registrar acknowledgement/failure evidence. Acknowledgement is not confirmation; non-transactional registrar/DNS work leaves through the kernel outbox and the Integrator owns delivery mechanics. Database triggers and online-role grants refuse history rewrite. |
| `DomainObservation` | `mod_domains.domain_observations` | `dotmac-domains` optional module | Append-only, binding-scoped provider fact with verbatim statuses and separate observed/received time. It may arrive before a local command/service exists, never writes lifecycle state, and is rebuildable from registrar polling after callback loss. |
| `DomainIntent` / `DomainHold` / `DomainAttentionCondition` | `mod_domains.domain_intents`, `domain_holds`, `domain_attention_conditions` | `dotmac-domains` optional module | Immutable desired contact/nameserver/DNS revisions, owner-decided active holds, and durable reconciliation-required attention. Collections may request a consequence but never writes these rows; transfer-out requires an exact-content approval receipt and is refused by Domains when any hold remains. Generic release/allow-lapse remains refused in V1. |
| `HostingSpecification` / `HostingSpecificationVersion` | `mod_hosting.hosting_specifications`, `hosting_specification_versions` | `dotmac-hosting` optional module | Immutable technical hosting specifications with module-assigned monotonic versions, structural package rank, exact allowance facts, closed change rules, content digests and a structurally enforced previous-version/digest chain. Hosting derives change direction and applies the current version's rules; price and saleability remain with Subscriptions. A semantic `package_ref` is connector-mapped and never a provider identity. |
| `HostingService` / `HostingDesiredRevision` | `mod_hosting.hosting_services`, `hosting_desired_revisions` | `dotmac-hosting` optional module | Tenant-only hosting-account aggregate and append-only desired account/package revisions. Suspension and restoration have explicit pending states: emitting a provider command is deferred, never proof the external effect occurred. The provider correlation is an all-or-none, assign-once `(capability_binding_ref, provider_account_ref)` pair unique per tenant. Specification references are same-owner composite FKs. The online role has no aggregate UPDATE/DELETE; one fixed-search-path SECURITY DEFINER function enforces tenant GUC, expected version, immutable identity/pair, legal graph and exact desired/observation evidence. |
| `HostingCommand` / `HostingCommandOutcome` / `HostingAttentionCondition` | `mod_hosting.hosting_commands`, `hosting_command_outcomes`, `hosting_attention_conditions` | `dotmac-hosting` optional module | Immutable business commands and append-only outcomes, plus durable business attention. Integrator owns delivery attempts, backoff, leases and dead-letter state. Reconciliation appends a separate applied confirmation for every deferred consequence the independent observation confirms; a later desired package or inverse suspension command appends superseded evidence, and provider failure remains durable. Earlier receipts are never updated. |
| `HostingObservation` / `HostingObservationResource` / `HostingSuspensionLock` / `HostingRetentionHold` | `mod_hosting.hosting_observations`, `hosting_observation_resources`, `hosting_suspension_locks`, `hosting_retention_holds` | `dotmac-hosting` optional module | Immutable deduplicated panel facts and exact resource observations, including aggregate mailbox count only. An observation may remain uncorrelated (`hosting_service_id` NULL) while tenant scope stays mandatory; reconciliation derives correlation through immutable operation/account references without rewriting evidence. Reason locks freeze their allowed restorers when opened, and retention holds outrank delinquency suspension and termination. |
| `HostingTerminationApprovalEvidence` | `mod_hosting.hosting_termination_approval_evidence` | `dotmac-hosting` optional module | Immutable tenant-local mirror of the released final `approval.approved` event and its trusted source-event delivery identity. Hosting conflicts on reused source/request identity with different content, validates subject, fixed policy identity and exact `sha256:` content before requesting termination, and still emits a durable refused outcome if a retention hold is active. The provider request contains no approval reference; only an independent terminated observation completes the deferred consequence. |
...
| Aggregate metric evidence and current-point projection | `dotmac_analytics.service.record_batch` is the only normal writer of declaration snapshots, source receipts, observations and winning points; `rebuild_projection` repairs only `metric_points` from immutable observations and records digest evidence. Both mutate and flush in the caller transaction. Metric calculations remain with their declared source-domain owners. |
...
| Registered-domain lifecycle and interpretation of registrar observations | `dotmac_domains.service` is the sole writer: request registration/renewal/transfer, append desired contact/nameserver/DNS revisions, receive immutable observations/outcomes, decide requested consequences, and reconcile observations into lifecycle state. Guarded transfer-out is the only intentional relinquishment; deletion is observational after expiry/redemption. Provider callbacks cannot call the lifecycle writer. The kernel owns idempotency/inbox/outbox/audit mechanics; the Cloud assembly translates versioned contracts; Integrator owns every provider call, credential, retry, checkpoint and health fact. `TenantDomain`, pricing, billing, dunning, hosting and fulfillment remain separate owners (ADR-0030). |
| Hosting-account lifecycle and interpretation of panel observations | `dotmac_hosting.service` is the sole writer of technical specification publication, hosting desired revisions, reason-scoped suspension/restoration, retention holds, guarded termination, observation ingestion and reconciliation; aggregate row mutation is further confined to `mod_hosting.mutate_hosting_service`. `hosting.account.v1` deliveries are closed self-contained snapshots with no local row references or secrets. Provider acknowledgements and enqueued work do not assert external success: pending states converge only through immutable independently identified observations, which append final applied/superseded/failed outcomes without rewriting prior evidence. Integrator owns provider identity/configuration, wire I/O, retry, checkpoints and health; Cloud, Subscriptions, Orders, Collections and Approvals interact only through versioned commands/receipts and remain separate owners (ADR-0030). |
