...
packages/dotmac-banking/         optional tenant banking owner
  pyproject.toml                 distribution dotmac-banking; audit-complete,
  EXTRACTION.toml                ERP-first candidate, then Backoffice
  src/dotmac_banking/            configured institutions/accounts, immutable
                 statements and cash observations, policy-driven matching and
                 reconciliation evidence; independent `bk` lineage in
                 `mod_banking`. No provider, GL or payment owner ports.
packages/dotmac-tax/             optional tenant tax owner
  pyproject.toml                 distribution dotmac-tax; audit-complete,
  EXTRACTION.toml                ERP-first candidate; Sub supplies source facts
  src/dotmac_tax/                governed policy data, determinations, report
                 snapshots, obligations and return timeline; independent `tx`
                 lineage in `mod_tax`. No country/rate/calendar constants.
packages/dotmac-payroll/         optional tenant payroll owner
  pyproject.toml                 distribution dotmac-payroll; audit-complete,
  EXTRACTION.toml                ERP-first candidate, then Backoffice
  src/dotmac_payroll/            typed configuration/revisions, opaque employee
                 assignments, calculations and liabilities; independent `py`
                 lineage in `mod_payroll`. No People, Tax, Banking or GL import.
...
| `BankInstitution` / `BankAccount` | `mod_banking.bank_institutions`, `bank_accounts` | `dotmac-banking` optional module | Tenant-authored treasury masters and opaque `cash_account_ref`; no compiled bank/provider catalogue and no accounting FK (ADR-0041; [`banking-sources.md`](inventories/banking-sources.md)). |
| `BankStatement` / `BankStatementLine` / `CashAccountObservation` | `mod_banking.bank_statements`, `bank_statement_lines`, `cash_account_observations` | `dotmac-banking` optional module | Immutable normalized external and cash-owner observations, deduplicated by source/version and carrying evidence. Provider I/O and payment meaning remain outside Banking. |
| `MatchPolicy` / `MatchDecision` / `MatchAllocation` / `Reconciliation` | `mod_banking.match_policies`, `match_decisions`, `match_allocations`, `reconciliations` | `dotmac-banking` optional module | Configured suggestion policy, explicit exact allocations and preparer/approver-separated balance snapshots. Suggestions are never decisions. |
| `TaxAuthority` / `TaxJurisdiction` / `TaxCode` / `TaxRule` / `TaxRuleBand` | `mod_tax.tax_authorities`, `tax_jurisdictions`, `tax_codes`, `tax_rules`, `tax_rule_bands` | `dotmac-tax` optional module | Effective governed policy data. Authorities, codes, rates, bands, recognition bases and recoverability are rows rather than country enums/constants (ADR-0042; [`tax-sources.md`](inventories/tax-sources.md)). |
| `TaxDetermination` / `TaxDeterminationLine` | `mod_tax.tax_determinations`, `tax_determination_lines` | `dotmac-tax` optional module | Deduplicated source facts and immutable selected-rule/version amount evidence. Products retain ownership of invoices, payments and payroll inputs. |
| `StatutoryReportDefinition` / `StatutoryReportBox` / `TaxFilingObligation` / `StatutoryReport` / `StatutoryReportValue` | `mod_tax.statutory_report_definitions`, `statutory_report_boxes`, `tax_filing_obligations`, `statutory_reports`, `statutory_report_values` | `dotmac-tax` optional module | Configured report vocabulary/calendar and versioned aggregate snapshots; no authority form or due date is compiled into code. |
| `TaxReturn` / `TaxReturnEvent` | `mod_tax.tax_returns`, `tax_return_events` | `dotmac-tax` optional module | Prepare/approve/file/accept/reject/amend evidence with separation of duties and an append-only timeline. Portal transport remains outside the owner. |
| `PayComponent` / `PayStructure` / `PayStructureRevision` / `PayStructureRule` / `PayRuleBasis` | `mod_payroll.pay_components`, `pay_structures`, `pay_structure_revisions`, `pay_structure_rules`, `pay_rule_bases` | `dotmac-payroll` optional module | Tenant-authored components and immutable typed input/fixed/percentage revisions. Component codes are data and stored expressions are not executed (ADR-0043; [`payroll-sources.md`](inventories/payroll-sources.md)). |
| `EmployeePayAssignment` / `PayrollRun` / `PayrollCalculation` / `PayrollCalculationLine` | `mod_payroll.employee_pay_assignments`, `payroll_runs`, `payroll_calculations`, `payroll_calculation_lines` | `dotmac-payroll` optional module | Effective assignments to opaque employee references and source-evidenced line calculations with proration and separation of duties. People/attendance/tax remain external owners. |
| `PayrollLiability` / `PayrollLiabilitySettlement` | `mod_payroll.payroll_liabilities`, `payroll_liability_settlements` | `dotmac-payroll` optional module | Finalized employee-net and external obligations plus coverage observations. Banking executes no payment here and Accounting receives consequences through an adapter. |
...
| Bank accounts, statement/cash observations, matching and reconciliation | `dotmac_banking.service` is the sole candidate writer for configured masters, imports, explicit allocations and reconciliation decisions. Integrator/product adapters supply observations, Accounting owns cash-ledger state and projections, and Sub retains collection-account routing. The reference assembly does not compose `bk` and ERP remains authoritative until a sealed cutover (ADR-0041). |
| Tax policy, determinations, statutory reports and returns | `dotmac_tax.service` is the sole candidate writer for governed policy rows, selected-rule evidence, configured report snapshots, obligations and return transitions. Product applications own taxable source facts, Integrator owns authority transport, and Accounting owns postings. The reference assembly does not compose `tx`; ERP remains authoritative (ADR-0042). |
| Payroll calculations and employee/external liabilities | `dotmac_payroll.service` is the sole candidate writer for components/revisions, opaque assignments, runs, line evidence, liabilities and settlement coverage. People, Tax, Banking and Accounting are independent owners reached through application ports. The reference assembly does not compose `py`; ERP remains authoritative (ADR-0043). |
