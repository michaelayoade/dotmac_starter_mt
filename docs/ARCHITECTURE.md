...
| Platform audit events | `dotmac_kernel.audit.write_platform_audit_event` is the sole constructor/writer. Kernel owns the storage contract as `platform_audit_log.v1`; migration `0026_platform_audit_log` makes online history append-only, and the live verifier proves shape, FK/index, no RLS, tenant-role isolation, and `platform_api` `SELECT`+`INSERT` only. `dotmac-integration` verifies the prerequisite in `ig_0008`; `dotmac-entitlement-allocation` verifies it in `ea_0003`, after its separate ledger verification. Both declare audit-action vocabulary but never own or create this table. Current limitation: deleting a `PlatformAdmin` nulls actor attribution, so immutable actor snapshotting remains a separate unresolved hardening slice. |
| Integrator destination revisions | `dotmac_integration.destination_binding.establish_destination` owns an operator-established route; `reconcile_product_port_descriptor` is the sole writer of the product-owned descriptor projection. The product publishes one authenticated immutable fact (binding id, capability owner/summary, scope, paths, activation and digest); the thin assembly retrieves and pins it, then the named reconciler appends only when the digest changes. Provider input is not a parameter to either writer. `resolve_destination` is the canonical reader, and no assembly map or connector payload is a parallel routing authority. |
| Integrator inbox receipt identity and disposition | `dotmac_integration.ingress.record_batch` is the sole constructor. `provider_event_id` is stored once on the receipt and crosses later claims/requests from that row; a connector payload never supplies a second copy. `InboundEvent.disposition=record_only` closes non-deliverable provider evidence as processed with a typed consequence, while `deliver` leaves the receipt for the product-delivery owner. Redelivery can neither reopen nor reclassify the stored fact. |
