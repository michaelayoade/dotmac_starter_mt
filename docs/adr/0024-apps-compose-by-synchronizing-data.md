...
production-derived inventory classifies every live external capability as
`migrate`, `retire`, or `retain-temporarily`. A continuing capability has a
complete packet: connector distribution, typed product port, product
descriptor, named reconciler, secret mapping, mirror evidence, rollback plan
and local retirement gate. A retiring capability proves zero traffic and its
deletion gate. Temporary retention names an owner and an explicit exit gate;
it is a blocker, never a fourth terminal disposition.

A direct, versioned product-to-product API that synchronizes product-owned
data is not external provider transport. It may be recorded as
`retain-product-api` when it preserves the application-independence rules in
this ADR: typed observations, local decision ownership, no shared persistence
and a named local reconciler where a projection exists. Product-local
infrastructure drivers and operational endpoints may likewise be recorded as
`retain-product-local`. These classifications prevent the inventory from
manufacturing connector plugins for local controls; neither classification may
hide a provider client, provider credential, callback, scheduler, checkpoint or
delivery retry path in a product runtime.
