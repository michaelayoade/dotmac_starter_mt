...
>
> **Evidence basis:** kernel side, the `dotmac_starter_mt` `0.1.0a98`
> integration branch (open, unmerged, unpublished) —
...
> 0.1.0a98. ERP side, seams verified 2026-08-26 on branch
...
7. **Rule 26 gates the pin.** A version in `pyproject.toml` or on `main` is not
   evidence it is published or pinnable. ERP must not pin `0.1.0a98` until a
   ...
   `pyproject.toml` reading `0.1.0a98` on this branch is exactly the kind of
...
**S1 and S2 are AUTHORED, not published.** Both are in one open starter
integration PR whose kernel version is declared and unreleased, and rule 26
refuses a declared version as evidence of anything.

They are ONE release, not a train. `0.1.0a98` carries three independent
changes — the public engine-free transaction and fingerprint surface (S1), a
facet-admission security repair that this plan does not otherwise depend on,
and the instantiable `DatabaseRuntime` (S2). None depends on the others; they
ship together because none had been published separately, and three declared
versions with no oracle behind any of them is three chances to pin something
that does not exist rather than one.

| Step | Kernel | State |
|---|---|---|
| S1 + S2 | `0.1.0a98` | one integration PR: open, declared-unpublished |

S3 cannot start until a98 exists as a published, tagged artifact. Publication,
pinning, merging, staging and production work remain separately authorized.

### S1 — The public engine-free transaction and fingerprint surface — AUTHORED (kernel a98)
...
### S2 — The configurable kernel session/tenant runtime — AUTHORED (kernel a98)
...
**Purpose:** move ERP onto the published a98 kernel as a change that adopts
nothing, so that the adoption in S3 is reviewed on its own merits against a
pin that already works.

**Precondition:** a98 exists as a published, tagged artifact with an
...
the published a98 pin rather than carrying its own kernel expectations. Doing
...
import time and primes only `app.current_tenant`". a98 removes both halves of
...
artifacts together against a published a98. If this step finds itself editing
