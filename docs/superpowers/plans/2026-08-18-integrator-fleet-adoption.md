...
Inventory and type the three currently detected HTTP surfaces plus the SMTP
surface the count ratchet does not see:
...
The first three become provider-neutral Academy ports plus transport bindings.
The console proxy is not silently treated as connector debt merely because it
uses HTTP.

Acceptance: capability catalogue, descriptors, destinations, secret-name
mapping, reconcilers, replay fixtures, staging deployment and separate
retirement gates for ERP exchange and SMTP.
...
Sub, ERP and CRM applications are running. Academy is not running, Backoffice
is absent, and no Vendor CP application process was observed. Therefore:

- Sub WhatsApp receive may proceed to an isolated staging rehearsal once its
  three recorded blockers are cleared;
- Academy, Backoffice and Vendor CP cannot claim staging readiness;
