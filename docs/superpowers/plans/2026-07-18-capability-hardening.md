...
> **SUPERSEDED IN PART (module control-plane directive, 2026-07-18):** Michael: "The existing capability-hardening work should be evolved toward this manifest-driven model rather than building separate registries independently" — see `docs/superpowers/reviews/2026-07-18-module-control-plane-directive.md`. Task 1 (code registries) and Task 4 (permission RBAC) are ABSORBED into the module-control-plane program (steps 2-4: ModuleManifest/ModuleRegistry, manifest-declared permissions + audit actions, tenant entitlements + require_capability) and must be re-planned against ModuleManifest as the single declaration point — do NOT build the standalone registries sketched below. Tasks 2 (list_query), 3 (impact preview), and 5 (WCAG) remain valid orthogonal deliverables, schedulable within or after the program.
...
- **SEQUENCING (adoption review, 2026-07-18): this plan runs AFTER the control-plane-security plan** (`docs/superpowers/reviews/2026-07-18-adoption-review.md` items 1-5 — platform auth, RLS-in-dev, docs authority, one transaction owner, security baseline). Michael: "directionally right. Complete it after the control-plane work."
- **Governance-test standard (adoption review): AST/runtime inspection, NOT string occurrence.** Every grep/substring-based check this plan sketches (audit-action literals, permission-code literals, envelope codes, destructive-control detection, icon-only buttons) must be implemented against Jinja/Python AST (`jinja2 env.parse` for templates, `ast` module for Python) or runtime registries wherever the checked construct is parseable — string matching only as a last resort with a documented bypass analysis. The display-settings plan's `*_at` filter test sets the precedent (AST-based after review).
- **Scope additions (adoption review "Next architectural layer" — fold into tasks or add as Task 5b/7):** API versioning + pinned-OpenAPI contract snapshot/diff gate; standard list responses (items/page/limit/total/navigation — Task 2 delivers this, keep); idempotency keys for provisioning + externally retried mutations; optimistic concurrency (version fields or ETags) for contested resources; RFC 9457 Problem Details adoption OR an explicit documented freeze of the current envelope (decision for Michael, surface at planning refresh); audit emission moved into service-owned mutation paths (API and web routes cannot drift); lifecycle/state-transition services for tenants, users, sessions, custom fields, roles. Re-plan task breakdown before execution — the original 6 tasks predate these additions.
- Branch `capability-hardening` off main **after the control-plane-security plan merges** (baselines then: v0.7.0, ~450 unit+arch, 43+ integration). Integration via `TEST_DB_PORT=5437` (5433/5434 are production — never touch). PR-to-green-then-merge finish; force CI with `gh workflow run ci.yml --ref capability-hardening`.
