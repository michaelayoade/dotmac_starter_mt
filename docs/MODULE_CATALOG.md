...
| [`dotmac-customers`](../packages/dotmac-customers/README.md) | optional module | [`audit-complete`](../packages/dotmac-customers/EXTRACTION.toml) | [tenant · `mod_customers`](../packages/dotmac-customers/src/dotmac_customers/manifest.py) | atomic (all declared planes) | not installed here | not allowlisted | `0.1.0a1` | `>=0.1.0a91` | — | `dotmac_sub` |
...
| [`dotmac-kernel`](../packages/dotmac-kernel/README.md) | universal facility | [`historical-pre-rule`](../packages/dotmac-kernel/EXTRACTION.toml) | n/a | — | — | [dedicated workflow](../.github/workflows/release-kernel.yml) | `0.1.0a95` | — | `dotmac_starter_mt` | `dotmac_academy_app`, `dotmac_erp`, `dotmac_sub`, `dotmac_vendor_control_plane` |
...
| [`dotmac-usage`](../packages/dotmac-usage/README.md) | optional module | [`audit-complete`](../packages/dotmac-usage/EXTRACTION.toml) | [tenant · `mod_usage`](../packages/dotmac-usage/src/dotmac_usage/manifest.py) | atomic (all declared planes) | not installed here | not allowlisted | `0.1.0a1` | `>=0.1.0a91` | — | `dotmac_sub` |
...
- **Contract:** Own inbox queues, executable provider-neutral routing rules and immutable decision evidence, inbox-specific agent presence/capacity, durable FIFO queue admission and stored queue position, capacity-safe eligible-agent assignment, repeatable release/reassign/requeue history, fair per-queue dispatch, durable per-queue round-robin rotation state and workflow evidence. Eligibility arrives as opaque caller-supplied references; Workforce retains teams, skills, shifts and field availability. NOT conversations/messages/read cursors, tickets/work orders, transport/connectors, notifications or AI.
...
- **Contract:** Publish immutable offer versions and price children beneath a stable offer identity; record immutable effective-dated subscription contract versions and stable line lineage; resolve half-open periods, declared proration and rate units from BillingCadence; and emit exact pre-tax RatedObligationOutputV1 and money-free CommercialEntitlementProjectionV1 facts. NOT invoices, receivables, payments, settlement, allocation, tax, FX, collections policy, product access, licences, providers, usage metering, documents, or the general ledger.
