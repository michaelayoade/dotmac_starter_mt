...
| [`dotmac-connector-sub-accounting`](../packages/dotmac-connector-sub-accounting/README.md) | stateless protocol adapter | [`audit-complete`](../packages/dotmac-connector-sub-accounting/EXTRACTION.toml) | n/a | — | — | [connector allowlist](../.github/release-connectors.json) | `0.1.0a1` | — | — | `dotmac_erp`, `dotmac_integrator` |
...
| [`dotmac-deployment-control`](../packages/dotmac-deployment-control/README.md) | optional module | [`adopted`](../packages/dotmac-deployment-control/EXTRACTION.toml) | [platform · `mod_deploy`](../packages/dotmac-deployment-control/src/dotmac_deployment_control/manifest.py) | atomic (all declared planes) | not installed here | not allowlisted | `0.1.0a2` | `>=0.1.0a77` | `dotmac_vendor_control_plane` | — |
| [`dotmac-deployment-foundation`](../packages/dotmac-deployment-foundation/README.md) | universal facility | [`adopted`](../packages/dotmac-deployment-foundation/EXTRACTION.toml) | n/a | — | — | not allowlisted | `0.4.0a1` | — | `dotmac_erp` | `dotmac_integrator`, `dotmac_starter_mt`, `dotmac_sub` |
...
| [`dotmac-kernel`](../packages/dotmac-kernel/README.md) | universal facility | [`historical-pre-rule`](../packages/dotmac-kernel/EXTRACTION.toml) | n/a | — | — | [dedicated workflow](../.github/workflows/release-kernel.yml) | `0.1.0a104` | — | `dotmac_starter_mt` | `dotmac_academy_app`, `dotmac_erp`, `dotmac_sub`, `dotmac_vendor_control_plane` |
| [`dotmac-licensing`](../packages/dotmac-licensing/README.md) | optional module | [`adopted`](../packages/dotmac-licensing/EXTRACTION.toml) | [platform · `mod_licensing`](../packages/dotmac-licensing/src/dotmac_licensing/manifest.py) | atomic (all declared planes) | not installed here | [module allowlist](../.github/release-modules.json) | `0.1.0a2` | `>=0.1.0a100` | `dotmac_vendor_control_plane` | — |
...
| [`dotmac-runner-transport`](../packages/dotmac-runner-transport/README.md) | universal facility | [`audit-complete`](../packages/dotmac-runner-transport/EXTRACTION.toml) | n/a | — | — | not allowlisted | `0.1.0a2` | — | — | `dotmac_observability`, `dotmac_starter_mt` |
...
| [`dotmac-tax`](../packages/dotmac-tax/README.md) | optional module | [`audit-complete`](../packages/dotmac-tax/EXTRACTION.toml) | [tenant · `mod_tax`](../packages/dotmac-tax/src/dotmac_tax/manifest.py) | atomic (all declared planes) | not installed here | [module allowlist](../.github/release-modules.json) | `0.1.0a4` | `>=0.1.0a98` | — | `dotmac_erp`, `dotmac_sub` |
...
| [`dotmac-ui`](../packages/dotmac-ui/README.md) | presentation foundation | [`audit-complete`](../packages/dotmac-ui/EXTRACTION.toml) | n/a | — | — | [dedicated workflow](../.github/workflows/release-ui.yml) | `0.1.0a8` | — | `dotmac_academy_app`, `dotmac_erp`, `dotmac_sub` | `dotmac_crm` |
...
- **Owner:** Stateless provider-neutral advisory invocation, transcription/model-execution attempt evidence and advisory insight lifecycle, and this package's own execution-admission policy lifecycle (AIPolicy/AIPolicyVersion) governing which operation kinds its own operation/attempt tracking allows to run; the consuming product's own orchestration owner (for example, Sub's ai.intake) owns session, routing/orchestration policy and every workflow consequence
- **Contract:** Carry one advisory invocation and its result across the boundary; record opaque input observations and immutable transcription/model-execution attempts; create advisory insights and record acknowledgement/action evidence without executing a domain consequence. Business-capability admission — whether a domain-owned capability such as support.customer.identify.v1 exists or is permitted at all — stays domain-owned and reaches this package only as an opaque exposure reference; this package's own execution-admission policy (AIPolicy/AIPolicyVersion, which operation kinds its own attempt lifecycle allows to run) is a separate, package-owned decision unaffected by that domain ownership. Provider/model/request names are observation strings only. NOT workflow next-step, routing/orchestration authority, provider selection, credentials, endpoints, APIs, wire formats, prompts owned by a product domain, connector retries, domain mutation or autonomous decisions.
...
### [`dotmac-connector-sub-accounting`](../packages/dotmac-connector-sub-accounting/README.md)

- **Owner:** Sub invoice-accounting-sync feed transport: authenticated (X-API-Key) paged GET polling of Sub's invoices/accounting-sync/v2 feed through an engine-owned cursor, response and per-item shape validation, disposition/issues consistency checking, and translation into the provider-neutral invoices.accounting_sync.observation.v1 POLL observation, including wire-shape validation and verbatim forwarding of Sub's own digest_version/projection_digest fields.
- **Contract:** Implement dotmac-integration SPI >=1.3,<2.0 for connector key sub_accounting. Serve invoices.accounting_sync.observation.v1 in POLL only. Page GET /api/v1/invoices/accounting-sync/v2 on selfcare.dotmac.io with X-API-Key auth, a canonical {after_id, after_updated_at} cursor (both fields or neither), always-sent limit and optional updated_since; validate strictly-increasing (updated_at, source_invoice_id) page positions and refuse a non-monotonic page. Emit only source-derived fields (source_invoice_id, source_account_id, source_updated_at, source_kind, disposition, source_total_amount/currency, optional source_issued_at/source_due_at, issues with line_id renamed to source_line_id, contract_version, digest_version, projection_digest) — never organization_id or an idempotency_key-shaped field, which are the generic Integration engine's and the destination ObservationPortClient's responsibility. digest_version/projection_digest are Sub-computed and forwarded verbatim after wire-shape validation only (positive int; 64-lowercase-hex string) — this connector never computes or reconstructs a digest itself. Refuse a blocked observation with no issues, or a non-blocked observation carrying issues, before emitting. Own no database, product identity, destination-org mapping, delivery idempotency, retry/checkpoint engine or ERP-side consistency enforcement.
- **Evidence:** `audit-complete` from [`EXTRACTION.toml`](../packages/dotmac-connector-sub-accounting/EXTRACTION.toml); source mode `greenfield-after-inventory`.
- **Proven consumers:** —.
- **Candidate consumers:** `dotmac_erp`, `dotmac_integrator`.
...
- **Owner:** Tenant-scoped conversation identity, lifecycle, message ordering, channel-trait threading, message identity, append-only transport correlations, and per-operator read cursors
- **Contract:** Create one durable tenant conversation per declared thread; append ordered inbound, outbound, and internal messages with one canonical identity that replays exact redelivery and rejects key reuse with different content; support opaque product-supplied thread and message references separately from provider transport evidence; bind append-only global- or account-scoped transport message correlations to a tenant message, replaying the exact binding and rejecting reuse by another message, while a channel declaring transport scope none cannot create a correlation key or row; late-bind only an opaque transport observation reference by tenant-scoped message UUID with exact replay and conflict protection; transition open, pending, snoozed, and resolved state through expected-state commands; represent finite snooze with a timezone-aware deadline or explicit indefinite snooze with the typed UNTIL_REPLY value (omitted snoozed_until remains invalid), persisting the latter as status=snoozed and snoozed_until=NULL; wake an indefinite snooze only on inbound module-owned message activity; reopen a resolved thread on a new inbound message; maintain a monotonic opaque-actor read cursor; and expose frozen typed tenant-scoped conversation get/list and ascending message timeline reads, with only declared filters, bounded opaque scope-bound keyset cursors, timezone-aware cursor timestamps, and UUID tie-break ordering. Correlation evidence is not deleted through its message or tenant parent. A product retains subscriber/person and all other domain relationships, must reuse its logical supplied message reference across retries, and owns timed wake scheduling. Every mutation participates in the caller's transaction and flushes without commit or rollback. NOT provider transport, connector installation, webhook authentication, raw provider payloads, delivery/retry, contact resolution, routing, assignment, presence, queues, attachments, realtime, templates, AI intake, tickets, subscribers, leads, customers, or product consequences.
...
- **Contract:** Ingest a bounded provider-neutral runtime-health observation under an authenticated source identity supplied by the caller; retain immutable observation evidence with acceptance-time freshness provenance; project latest component state and fresh/stale/missing classification; build incident-ready summaries with explicit unknowns; author canonical signed DeploymentHealthEvidence through the authoritative produce_signed_health_evidence service entry point and an assembly-supplied HealthEvidenceSigner; and rebuild projections deterministically, refusing any selected legacy observation whose provenance is unknown until a newer observation arrives. NOT raw telemetry, metric/log/trace storage, network monitoring, alert delivery, incident case lifecycle, key material or cryptography, provider transport, or any deployment/configuration mutation.
