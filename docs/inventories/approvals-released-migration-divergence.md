...
`ap_0001_approvals` was published under four tags with three different byte
sets before `dotmac-approvals` entered the released-migration guard:
...
The tree retains the a3/a4 digest as the canonical released bytes. The guard in
...
`outbox_relay.v1` prerequisite and never edits or reruns `ap_0001`.
...
4. a3/a4 tenant-only;
5. a3/a4 platform-only;
6. a3/a4 with both planes.
