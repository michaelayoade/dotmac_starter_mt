...
**Implementation status addendum — 2026-08-19.** The coordinated Billing
candidate now implements `InvoiceDocumentFactV1`, including payment
instructions and immutable presentation snapshots, plus the typed
`RecordDocumentArtifactV1` official-artifact relation and its supersession
proofs on both planes. That closes the producer-shape and relation mechanics at
candidate-code level; it does not release or adopt them. Billing still exposes
no named public reader for issued document facts, the qualifying ERP source
review below is still open, and ADR-0017's demand gate still holds. No
`dotmac-document-rendering` package exists.
...
next_action = "1. Review the coordinated Billing candidate's InvoiceDocumentFactV1 and RecordDocumentArtifactV1 against all seven producer requirements in the contracts spec; payment instructions, presentation snapshots and artifact supersession now exist as candidate code, but release/adoption evidence does not. 2. Add or name Billing's public issued-document-fact reader before an assembly reconciler exists; the current package exports the fact type and write paths but no public fact reader, and a renderer may never compensate with a cross-module table read. 3. Read dotmac_erp's DocumentGeneratorService, GeneratedDocument and its 10 integration tests, which docs/inventories/template-studio-source-audit.md names as document generation's qualifying source and which this audit has NOT read; source_paths and preserved_tests remain placeholders until verified, and the ERP source may change which implementation is product-first. 4. Reword the billing spec's section 2.5 invariant 2 from re-render byte-for-byte-equivalent to semantically equivalent under the canonical semantic projection; the fact-side replay test it names is correct and needs no change. 5. Nothing else starts until an explicit ADR-0017 demand gate opens P8a. No package directory, package-root EXTRACTION.toml, namespace allocation, model or migration lands merely because Billing now has candidate producer seams."
```
