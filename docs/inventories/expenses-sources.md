...
`dotmac_erp` is the qualifying source and first consumer. It pins the exact
kernel and module releases, installs the `ex` lineage in its own database, and
uses a typed local adapter while the legacy expense implementation remains sole
writer. Backfill categories, effective policy revisions, requests, claims,
lines, receipt metadata and evaluation/lifecycle evidence, then compare counts,
stable references, status, totals, receipt completeness and eligibility for a
fixed shadow window. Backoffice follows only as an independent second consumer
of the release ERP has already proven.
