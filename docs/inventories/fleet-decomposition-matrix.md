...
**As of:** 2026-08-18 positioning classification amendment over the frozen 2026-08-12 source snapshot
**ERP:** `0f4b1698` (`origin/main`) · **CRM:** `c64b5aa0` (`main`) · **Sub:** `9f6f9f36` (`feat/kernel-pin-a40`) · **Vendor CP:** `eb667fa` (`main`) · **Starter:** `92ae7a6`
...
| engagement-inbox | 0 | 27 | 29 | 0 | 0 | 9 | consolidate → Sub, then module ← Sub |
...
| positioning | Sub `field_tech_location_pings` + mixed `field_tech_presence`; CRM's fork plus `crm_agent_location_pings`; ERP vehicle GPS placeholders and attendance geometry | Sub/CRM field stack is one forked lineage, not two consumers; ERP has no observation writer | Sub field operations first; ERP fleet second; future tracked-unit applications | Each adopter runs its own tenant-plane `mod_pos` lineage; product-owned tables link a local subject to an opaque tracked-unit id | Current Sub/CRM presence rows mix workforce status, disclosure policy and current position. ADR-0032 separates only observation/current/trail/geofence facts; products retain subjects and consequences. Static map/GIS/plant coordinates remain with their domain owners. | `dotmac-positioning` module | Correct Sub defects D1–D7 from [`positioning-sources.md`](positioning-sources.md), shadow and cut Sub over, retire its local writer, then adopt in ERP through a vehicle link. Extract map presentation only after both products consume the released read contract. |
...
| engagement-inbox | 27 | 29 | 9 aliased | The largest duplication in the fleet, and the one exact-name counting scored as zero: `crm_conversations`/`inbox_conversations`, `crm_messages`/`inbox_messages`, `crm_agent_presence`/`inbox_agent_presence`, `crm_pipelines`/`pipelines`, plus the routing/macro/assignment tables. `crm_agent_location_pings` moved to the narrow positioning family under ADR-0032; online/contact-centre presence remains here. Sub already owns `conversation_ticket_handoff` and `conversation_lead_relationships`, so the seam is load-bearing today. Retires when Sub's team inbox closes CRM's agent-facing capability gaps. |
| positioning | 3 | 2 | 2 exact | CRM's `field_tech_location_pings`/`field_tech_presence` duplicate Sub exactly; CRM additionally has browser `crm_agent_location_pings`. Consolidate corrected field behavior into the product-first module, not into a third Sub-local implementation. The mixed presence table splits at cutover: workforce status/sharing policy stays in Sub; current coordinates move to the module projection. |
| field-workforce | 24 | 36 | 13 exact | Work orders, technician profiles, shifts, skills, dispatch and ETA are Sub-owned; CRM keeps only agent-side projections, provenance-stamped and repairable. Position observations/current projection are now the separate ADR-0032 unit. **Known gap:** Sub's dispatch assigns without availability/conflict checks, and CRM's `is_technician_available` dies at exit — close it before, not after. |
...
| **1** | CRM → Sub authority consolidation, driven by Sub's existing `crm_web_retirement` ledger: outside-plant, engagement-inbox, field-workforce, sales-agreements, geospatial, subscriber projections. **Entry gate:** declare an owner in Sub's registry for the 28 duplicates that have none — see [`fleet-fact-level-decomposition.md`](fleet-fact-level-decomposition.md). | 70 of the 133 CRM↔Sub duplicated names after ADR-0032 moved two exact location duplicates into positioning, and it needs no new package to start. The authority questions are already adjudicated; this is capability closure and reconciliation against a named owner. Intermediate — these domains follow into modules in wave 4. |
...
| **2P** | `dotmac-positioning`: correct Sub's location contract/privacy/idempotency/retention defects, extract product-first, shadow and cut Sub over, then adopt in ERP. | ADR-0032 settles the cross-domain unit and [`positioning-sources.md`](positioning-sources.md) names both real adopters. It follows the first module-lineage proof but precedes broader workforce/fleet extractions that would otherwise rebuild its mechanics. Shared map UI remains gated on both cutovers. |
