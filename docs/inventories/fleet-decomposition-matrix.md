...
| Wave | Work | Why here |
|---|---|---|
| **0** | Finish kernel lineage adoption: Sub on `0.1.0a40`, settings task #22, ERP's E8 tenancy gate. CRM has **no kernel pin and zero `dotmac_kernel` imports** — it needs an entry plan, not a module. | ADR-0017. No PRODUCT runs a non-kernel module lineage — the `dotmac_integrator` assembly runs `mod_intg`, but an assembly is not an adopter; every later wave is blocked behind this. |
