...
| field-workforce | 26 | 38 | 15 exact | Work orders, technician profiles, shifts, skills, dispatch and ETA are Sub-owned; CRM keeps only agent-side projections, provenance-stamped and repairable. The narrower physical-execution owner now exists as the audit-complete `dotmac-work-orders` candidate (ADR-0031), with no product cut over; workforce roster, availability, routing and location deliberately remain outside it. **Known gap:** Sub's dispatch assigns without availability/conflict checks, and CRM's `is_technician_available` dies at exit — close it before, not after. |
