...
| `dotmac-publishing` | `product-first` | `dotmac_mkt` | Gate 1 RED is recorded and the Gate 2 candidate now owns immutable releases, one local delivery per opaque target, monotonic attempts, normalized observations and explicit partial/all-failed reconciliation. Preserve the qualifying `PostDelivery` scheduling behavior while replacing direct provider adapters with kernel outbox intents and an assembly-supplied typed timer port. Kernel a74 and publishing a1 remain local, unpublished, unallowlisted, uncomposed and unadopted pending exact-commit Observer proof. |
