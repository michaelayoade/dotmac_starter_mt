...
| Catalog discovery card sources | Workspace application discovery and Academy course discovery as independent sources of the display-only `dotmac-ui` catalog grid, plus the recurring-offer owner-read boundary | `catalog-card-sources.md` |
| Web analytics sources | Fleet-wide first-party and provider-aggregate inventory, greenfield ruling, privacy/retention boundary, and Backoffice-first/Sub-second adoption evidence for `dotmac-web-analytics` | `web-analytics-sources.md` |
| Analytics sources | Eight-repository product-first audit for `dotmac-analytics`: ERP's scalar metric projection is the qualifying source; domain calculations stay local; direct cross-domain ORM/data-explorer topology is negative evidence; web analytics and media observations remain separate owners | `analytics-sources.md` |
| **Positioning sources** | Sub/CRM field-location evidence, ERP fleet/attendance requirements, the location-vs-map-vs-business-policy boundary, defects that must not be ported, and the Sub-first/ERP-second cutover contract accepted by ADR-0036 | `positioning-sources.md` |
| Sales sources | Sub's qualifying lead/Pipeline/Quote owner, CRM retirement lineage, accepted-Quote boundary, parity canaries and Sub-first handoff cutover | `sales-sources.md` |
| Conversation/inbox sources | Sub's staffed-inbox aggregate, CRM retirement evidence, ERP candidate demand, and the conversation-vs-transport/workforce boundary | `inbox-sources.md` |
| Surveys sources | Sub's typed feedback lifecycle, CRM ancestry/delivery evidence, ERP candidate demand, and the feedback-vs-subject-consequence boundary | `surveys-sources.md` |
| Projects sources | Sub's qualifying work structure, CRM retirement evidence, ERP candidate requirements, dependency scheduling corrections and Sub-first cutover | `projects-sources.md` |
| Work-order sources | Sub physical-execution authority, CRM retirement-only evidence, the product consequence boundary, and the unresolved Vendor InstallationProject exclusion | `work-orders-sources.md` |
...
| **Cloud module implementation status** | as-of ledger separating committed implementations, active worktree WIP, source-audit-only owners, releases and real adopters; records the current Billing/Orders/Subscriptions/Collections train, Domains WIP, and the remaining Hosting/Fulfillment/Document Rendering construction work | `cloud-module-implementation-status.md` |
