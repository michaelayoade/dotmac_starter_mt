...
| **Integrator adoption ledger** | the executable distinction between one capability cutover and shared-plane retirement: Sub's current connector catalogue, pinned Academy/ERP/CRM source maps, per-capability migrate/retire/retain dispositions, eight-part migration packets, the seven shared tables, typed environment snapshots, Seabone's measured staging population, ERP's staging-derived migration/financial/survivor proof, Academy's exact-image `0049`→`0053` empty-staging proof, survivor gates and the fleet-zero condition | `integrator-adoption-ledger.md` + `integrator-adoption-ledger.json` + `integrator-{academy,erp,crm}-source-catalogue.json` + `evidence/integrator-adoption-seabone-{academy,erp}-*-2026-08-19.json` |
