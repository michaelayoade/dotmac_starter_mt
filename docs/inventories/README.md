...
| **Decomposition current state** | the 2026-08-19 exact-revision refresh for Sub, ERP and CRM: current package coverage versus real adoption, the fourteen new Sub tables that block a blind ratchet refresh, active concurrent module work, the remaining owner audits, and the decision to extend UI contract-by-contract rather than split another UI package | `decomposition-current-state.md` |
