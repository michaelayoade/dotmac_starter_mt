...
| **Provider capability and conformance** | five rulings — the connector framework already exists in `dotmac-integration`, PSP is product-first with a complete audit, registrar/DNS/panel are greenfield — plus the finding that **zero Blesta code exists in the fleet**, the four capability surfaces specified as shapes, and the conformance-kit obligations. Its dated 2026-08-17 note records the held-secret resolver and ADR-0033's narrow seven-distribution authorization without rewriting the 2026-08-15 evidence. | `provider-capability-sources.md` |
| **Managed-service connector sources** | the Rule-24 audit for Keycloak Admin, ERP, Mailcow, Nextcloud, Academy, Contabo IaaS, authoritative DNS/PTR and the constrained host agent: exact qualifying sources/tests, greenfield surfaces, minimum owner contracts, the no-secret-output boundary and the gates that remain after ADR-0033 names the seven distributions | `managed-service-connector-sources.md` |
