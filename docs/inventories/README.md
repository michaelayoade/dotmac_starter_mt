...
| Work-order execution sources | Sub's authoritative internal-crew WorkOrder lifecycle and evidence path, CRM's retiring duplicate, ERP's distinct fixed-asset maintenance meaning, the dispatch/workforce concerns excluded from the package, and the unresolved separation between WorkOrder execution and the vendor InstallationProject commercial wrapper | `work-orders-sources.md` |
