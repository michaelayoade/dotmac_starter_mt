...
| Party module sources | CRM/ERP/Sub identity models audited for the audit-complete `dotmac-party` candidate: Sub is the qualifying source; ERP/CRM are requirement inputs; ADR-0019 resolved the `party_roles` collision and role-to-role relationship seam; the package corrects the tenancy/open-vocabulary deltas; Sub's backfill, reader cutover and writer retirement remain binding | `party-module-sources.md` |
