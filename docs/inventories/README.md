...
| **Fleet decomposition matrix** | the standing ERP/CRM/Sub capability matrix PRODUCT_VISION § 2 requires — owner, competing writers, consumers, migration owner, authority overlap, target layer and retirement condition per capability, plus the frozen duplication baseline and the sequencing it implies (duplication orders the work; every domain still resolves to kernel, UI, or a Starter module) | `fleet-decomposition-matrix.md` |
