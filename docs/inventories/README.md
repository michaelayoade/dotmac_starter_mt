...
| **Procurement sources** | ERP as the sole qualifying generic buyer-side requisition/RFQ/offer/evaluation/purchase-order source; Sub and CRM project-vendor bidding retained with the product owner; Academy content, Backoffice intent and Vendor CP seller-side contracts ruled out; plus the Inventory, Approvals and Payables boundaries and sealed ERP-first cutover | `procurement-sources.md` |
