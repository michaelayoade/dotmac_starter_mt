...
| Omni-channel inbox sources | Sub's and CRM's inbox implementations, why CRM is a retirement source and not a second consumer, the observation-vs-decision gap between them, the hardcoded channel-name sets that reduce to four declared traits, ERP's orphaned email-helpdesk fields, and why the extractable unit is a conversation record rather than an inbox | `inbox-sources.md` |
