...
| **Paystack connector dossier** | the product-first, ingress-only `dotmac-connector-paystack` slice: exact-byte HMAC-SHA512 verification, `charge.success` settlement observations, provider-neutral evidence, deny-all egress, and a Sub-first shadow/callback cutover that leaves every financial consequence with the product owner | `../../packages/dotmac-connector-paystack/EXTRACTION.toml` |
| **Flutterwave connector dossier** | the product-first, Flutterwave-v4-only `dotmac-connector-flutterwave` slice: exact-byte HMAC-SHA256 verification, v4 `charge.completed` observations, no v3 fallback, no invented provider fee, deny-all egress, and a Sub v3-to-v4 callback cutover that leaves every financial consequence with the product owner | `../../packages/dotmac-connector-flutterwave/EXTRACTION.toml` |
