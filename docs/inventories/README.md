...
| **Analytics sources** | the eight-repository product-first audit for `dotmac-analytics`: ERP's production-used scalar metric projection is the qualifying source; domain computers and decisions stay local; Insights' direct cross-domain ORM/data-explorer topology is negative evidence; web analytics and media observations remain separate owners | `analytics-sources.md` |
