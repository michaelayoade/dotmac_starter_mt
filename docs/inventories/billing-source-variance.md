...
| **Receivable** — `ReceivablePositionV1` | **Yes twice, incompatibly** | No | **BLOCKED, two ways.** G2: billing publishes identity `(scope, billing_account_id, currency)` + `as_of_version`, third field `prepaid_funding`, as a **published fact**; collections specifies identity `(source_owner, exposure_ref, source_version)`, third field `funding_available`, delivered by a **synchronous `ReceivablesReader` port** with `Ok/Unavailable/Unknown/AuthorityMismatch`. *"Neither document acknowledges the other."* G1: billing's position **carries no service period**, but `dotmac_sub:app/services/collections/prepaid_policy.py:57` reads `period_start` to enforce "the service period has not started" — without it prepaid collections manufactures cases for future periods |
