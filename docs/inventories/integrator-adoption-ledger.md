...
**As of:** 2026-08-19  
**Starter:** `6893927` (`origin/main`)  
**Sub catalogue source:** `0d27ab91`,
`app/services/integrations/registry.py`  
**Runtime evidence:** read-only Seabone staging snapshots for Sub, ERP, CRM and
Academy, plus staging-derived clone migration rehearsals for ERP and Academy.
The uncommitted ERP and Academy candidates are bound by base revision, patch
digest and exact image digest; they are staging evidence, not releases or
production claims.
...
| Academy | Complete seven-surface staging census plus exact-artifact migration proof | mixed | Capture the same seven surfaces on a named production target and build the SMTP packet; retain the three typed ERP APIs and three local operational surfaces |
...
have exact source maps at pinned revisions. Academy's complete staging census
classifies all seven surfaces, but it and the other products remain explicitly
incomplete until the same full surface is captured from a named production
target. Staging absence never becomes production zero.
...
Academy's stopped Seabone staging volume was copied read-only into an isolated
volume, then migrated by the exact candidate image from
`0049_enrollment_audience` to `0053_entrance_defaults`. The source had zero
tenants and zero business rows; all six measured table populations were
unchanged, including the two platform settings. The resulting complete census
found SMTP and the lab-console proxy configured, while the three typed ERP
flows, GlitchTip and Prometheus were explicitly absent. This proves the
candidate's migration, complete observation shape and row preservation on
staging. It does not prove production absence, billing behaviour or cutover
readiness.
...
## Academy product capture adapter

Academy now has an uncommitted candidate owner,
`integrator_adoption_inventory`, at source revision `a5e25e4e`. It emits all
seven mapped surfaces exactly once from one `app_admin` repeatable-read,
read-only transaction and proves that the transaction rejected a write. It
reports only aggregate counts and configuration presence; it never loads a
network client or emits configuration values, addresses, payloads, headers or
secret material.

The exact Seabone candidate image passed six focused PostgreSQL tests, the full
Academy suite (`946 passed, 2 skipped`), Ruff, mypy, pip-audit and the Node 20
CSS build with an unchanged compiled stylesheet digest. Its staging-derived
clone reached current head and produced the seven-row census described above.
Production remains unmeasured until this owner is merged, deployed to an
explicitly named target and invoked there using the same exact-artifact
capture contract.
