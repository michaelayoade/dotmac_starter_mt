...
[`ADR-0069 — Mobile authentication federates to the existing identity provider`](../adr/0069-mobile-authentication-federates-to-the-existing-identity-provider.md)
...
| R4 | the Keycloak realm | The realm advertises `code_challenge_methods_supported: ["plain", "S256"]` and `grant_types_supported` including `implicit` and `password`. `plain` PKCE is not a proof of possession. These are **realm-level** advertisements; the mitigation is per-client and does not exist yet. | `https://idp.dotmac.io/realms/dotmac/.well-known/openid-configuration`, fetched 2026-08-26 (HTTP 200) | ADR-0069 § 4 — per-client `S256` pin and per-client disabling of `implicit`/`password`, both prerequisites |
