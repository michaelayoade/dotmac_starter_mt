...
publication are different questions, and this detector owns only the last.
...
### Historical finding, measured 2026-08-15

This table is the measurement that caused the detector to be built, not a
current publication inventory. Current state is generated by
`python3 scripts/declared_publication_sweep.py --check` and reconciled against
`declared-publication-baseline.json`; copying that moving inventory into this
policy would create a second publication ledger.

| Distribution | Declares | Newest tag | State |
|---|---|---|---|
| `dotmac-integration` | `0.1.0a2` | `0.1.0a1` | **declared-unpublished** |
| `dotmac-imports` | `0.1.0a2` | — | no verified publication tag, *intended* (allowlist row removed) |
| `dotmac-auth-oidc` | `0.1.0a1` | — | no verified publication tag, *deliberate* |
| `dotmac-template-studio` | `0.2.0a3` | — | no verified publication tag, no lane |
| the other eight | — | = declared | published |

At that measurement, `dotmac-integration` was the programme's example and was
recorded as **evidence**, not repaired by editing a version down. `0.1.0a2` was
the 2026-08-14 strictness fix while the latest verified publication was
`0.1.0a1`, whose `run_effect_once` raised `TypeError` on its first call. The
release decision belonged to the release step — a guard that had "fixed" this
by changing a number would have deleted the evidence that decision needed.

`dotmac-imports` was the sharper case and was initially **resolved — in the
opposite direction from the one the report suggested.** It was
release-allowlisted with no
...
That historical hold was later discharged: the 2026-09-05 sweep reports
`dotmac-imports 0.1.0a2`, `dotmac-auth-oidc 0.1.0a2`,
`dotmac-template-studio 0.2.0a4` and `dotmac-integration 0.1.0a17` as matching
verified tags. Those coordinates live in the generated/tag-backed inventory,
not in the historical table above.
...
permission, not the artifact. At this historical point the ledger distinguished
*intended* unpublished states (`dotmac-auth-oidc`, `dotmac-imports`) from an
*outstanding* one (`dotmac-integration`). Current rows and their current reasons
come only from `declared-publication-baseline.json`; the reason text is where
that difference lives, which is why the guard demands a premise rather than a
label.
...
version with no verified publication cannot be resolved from the repository's
release record, so it is not a floor at
