...
| `dotmac-accounting` | **Audit-complete; zero contract consumers** | ERP is the sole qualifying production source for chart, fiscal periods, journal posting, reversal and ledger evidence. The extraction ports its behavior and parity tests while removing product commits/rollbacks, fixed dimension columns and product ORM coupling. Sub's customer subledger is explicit evidence that operational postings do not make Sub a GL owner. Full evidence in [`accounting-payables-sources.md`](accounting-payables-sources.md); ownership is ADR-0041. | Keep unallowlisted until ERP is ready to pin kernel a73 plus Accounting a1, backfill and shadow the full chart/period/journal/ledger contract, reconcile it, and seal the writer cutover. Payables consequences activate only after this owner is live. |
| `dotmac-payables` | **Audit-complete; zero contract consumers** | ERP is the sole qualifying production source for supplier invoices, credits, liabilities and obligations. The extraction retains document and subledger behavior but leaves supplier identity, Procurement, tax, inventory, Accounting and Treasury/payment execution with their owners. Full evidence in [`accounting-payables-sources.md`](accounting-payables-sources.md); ownership is ADR-0042. | Keep unallowlisted. After ERP's Accounting cutover, pin Payables a1, shadow invoices/credits/liabilities/obligations and consequence receipts, reconcile them, then seal and retire the ERP AP writer. |
