...
| `dotmac-analytics` | **Audit-complete; zero contract consumers** | ERP's `OrgMetricSnapshot`, scheduled computers, `MetricStore` and dashboard/coach readers are the qualifying source for exact scalar aggregate projections. The shared boundary keeps calculations in their domain owners and ports latest/history/comparison behavior with declaration, source-evidence, tenancy and repair hardening. Sub's generic KPI calculator is inert; Insights' direct cross-domain ORM and data explorer are decomposition evidence, not source code. Full evidence in [`analytics-sources.md`](analytics-sources.md); ownership is ADR-0036. | Prove the package on Observer, then shadow ERP one metric-computer family at a time and retire its snapshot/store writer and reader paths after parity. Backoffice is consumer two. Saved dashboards/report schedules remain deferred pending a separate reporting-owner audit. |
