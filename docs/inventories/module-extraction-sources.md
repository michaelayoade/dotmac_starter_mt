...
| `dotmac-procurement` | **Audit complete; a1 built and uncomposed** | ERP is the only qualifying generic buyer-side source for requisitions, RFQs, supplier offers, evaluations and purchase orders. The product-first port preserves its guarded draft/submit/approve behavior and exact line totals, while refusing source defects: mutable bids, incomplete criteria, date-only windows, product FKs, direct AP writes and over-receipt. Sub/CRM project-vendor bidding remains product-owned. Full evidence in [`procurement-sources.md`](procurement-sources.md); ownership is ADR-0038. | Validate the exact branch commit on Observer, then compose exact kernel a73 and procurement a1 in ERP behind its Organization-to-Tenant mapping. Replay/shadow every decision and switch authority only through the ADR-0031 sealed transaction. Keep the package unpublished and Backoffice/Sub uncomposed until that first writer cutover is ready. |
