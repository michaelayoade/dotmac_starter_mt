...
| `dotmac-template-studio` | **Audit-complete; zero contract consumers** | The audit rejected the package's two-kind merge and selected Sub as the source for the notification-only contract. The package now owns only "what the message says": its single-brace renderer carries Sub's seven parity tests, and its seeder carries Sub's `(code, channel)` upsert, never-clobber and executable-spec derivation rules. ERP documents remain outside this module. Full evidence in [`template-studio-source-audit.md`](template-studio-source-audit.md); accepted boundary in ADR-0006 § 5a–5c. Next gate: keep it unreleased until the deferred cohort cutover window and retire the asserted no-`DATABASE_URL` import exemption before allowlisting; then publish an exact pin just in time, shadow-render Sub, and retire Sub's renderer and seeder together. |
