...
| `dotmac-publishing` | product-first from Mkt post-delivery lifecycle and partial-success behavior | Gate 0 freezes the immutable-snapshot, per-target attempt, normalized-observation, timer and typed-outbox contract. Run the checked-in Gate 1 canaries RED before allocating `publishing`/`pb`/`mod_publishing` on the next unclaimed kernel alpha. |
