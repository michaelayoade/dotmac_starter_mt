...
| `dotmac-finance` | **Audit complete; a1 candidate is uncomposed and unpublished** | ERP is the only qualifying fixed-asset accounting source. The package preserves depreciation, valuation and disposal-accounting behavior while separating physical Assets, GL/fiscal-period ownership, tenant isolation and transaction authority. Full evidence in [`finance-asset-accounting-sources.md`](finance-asset-accounting-sources.md); ownership is ADR-0038. | Validate the exact revision on Observer, then compose in ERP for backfill/shadow only. Publish just in time for the sealed ERP authority switch after zero unexplained book/consequence drift; Backoffice is the second independent candidate. |
