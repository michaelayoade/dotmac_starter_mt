...
| `dotmac-ui` | **Audit-complete by slice; tokens and empty-state reuse-proven** | Its semantic-token vocabulary started from Sub's live `static/css/design-system.css`. Tokens are consumed by Sub and Academy; the empty-state component is consumed by ERP and Sub. Map Frame and Catalog Grid are audit-complete with no adopter. ERP's live local CSS remains retirement debt. | Adopt Map Frame and Catalog Grid before broadening the component surface. Then extract Sub's list values into kernel and add token-native list rendering in UI; keep CSV parsing in Imports and product composites outside UI. Current evidence: [`decomposition-current-state.md`](decomposition-current-state.md#ui-decomposition-recommendation). |
...
| `dotmac-orders` | [`orders-sources.md`](orders-sources.md); customer-order aggregate and fulfillment-eligibility decision only | Revalidate the completed Sales→Orders contract on Observe, then map and shadow Sub's existing finite funding receipts |
