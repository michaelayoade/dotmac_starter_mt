...
| `dotmac-party` | **Audit complete; candidate implemented but intentionally unreleased and uncomposed** | Sub is the qualifying source for Party business context; ERP/CRM are requirement inputs, while kernel Party remains identity. The five-table `mod_party` tenant plane corrects Sub's tenancy and external-reference uniqueness defects, replaces closed product vocabularies with owner declarations, and resolves ambiguous bare-Party relationships to exact PartyRole endpoints. Full evidence in [`party-module-sources.md`](party-module-sources.md) and ADR-0019's 2026-08-19 amendment. | Keep the release lane closed. Sub must finish its reviewed backfill and reader cutovers, adapt its relationship projection to role endpoints, prove shadow parity, and retire the five local writers under a sealed switch before this package can be published or called adopted. Backoffice remains a candidate, not evidence of reuse. |
