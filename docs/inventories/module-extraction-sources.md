...
| `dotmac-media-observations` | **Audit-complete and candidate-validated; zero contract consumers; adoption paused** | Mkt is the qualifying behavior/test source for normalized campaign/group/advertisement hierarchy and daily aggregate performance, but production use is unverified. CRM supplies negative evidence against treating provider claims as attribution; Sub remains authoritative for immutable Lead origin and customer lifecycle; Integrator owns transport. The tenant-only package, clean wheel and PostgreSQL lineage passed Observer and CI; full source and validation evidence is in [`media-observations-sources.md`](media-observations-sources.md), with ownership in ADR-0032. | Keep it uncomposed and outside the release allowlist until Michael resumes adoption. Verify Mkt runtime authority, then Backoffice performs the first authorized shadow/reconciliation/cutover proof and PR-only governance; publication follows that proof, and Sub follows independently. |
...
| `dotmac-sites` | greenfield after an exact six-repository audit; no qualifying source and zero consumers | Gate 2 is Observer-green at `8bf12ddc5cb938714d090fc0b0e69b83fa78f2d2`, with final evidence at `e0034342f904cfc26bae94a5fd4be8650424f376`. Keep the a1/a77 candidate unpublished, unallowlisted, uncomposed and unadopted; remote publication additionally requires the typed publishing/Integrator boundary. |
...
| `dotmac-web-analytics` | greenfield after the fleet audit found no qualifying first-party observation owner; Mkt's provider aggregates and CRM's identity-bearing tracker are requirements/negative evidence only | Reconcile the committed Starter implementation at `abef05ac5fd121ca254bb74eafcc7c9970e90dfd` onto kernel a78, prove its typed privacy contract, deterministic rebuild and forced-RLS lineage on Observer, and keep Backoffice/Sub adoption, release and local-writer retirement separately gated. |
