...
- `app/features/<name>/` — self-contained: `models.py`, `schemas.py`,
  ...
  UUID columns, never a Python import.
...
- Every tenant-scoped model: `tenant_id UUID NOT NULL REFERENCES tenants(id)`
  ...
  migration that creates the table. Not statically checked — enforced by the
  ...
  which fail if isolation is missing. Run these against real Postgres
