"""Verify composition helper bytes before importing the contract package.

This file deliberately imports no module from ``tools.composition_contract``.
The compatibility CI job executes it in a separate process before pytest can
import ``composition_schema``, ``observations`` or ``specs``. A check performed
after those imports could detect on-disk drift, but could not prove which bytes
...
def main(arguments: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--revision", default=TRUSTED_CONTRACT_REVISION)
    options = parser.parse_args(arguments)
    try:
        drift = find_source_drift(options.repository, revision=options.revision)
    ...
    if drift:
        print(
            ...
            f"{options.revision}: {list(drift)!r}"
        )
        return 1
