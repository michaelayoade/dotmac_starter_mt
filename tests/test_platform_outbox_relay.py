"""Platform outbox relay behavior on real Postgres (WS3 platform relay).

The platform peer of `test_outbox_relay`: exercises the typed claim/success/failure
operations over the least-privilege `platform_outbox_dispatcher` connection against
the 0012 SECURITY DEFINER functions. Proves at-least-once with **one active claim
per lease** (concurrent workers never claim the same row), retry/backoff/dead-letter,
and stale-lease recovery. Platform events carry NO tenant. Requires Postgres (0012).
"""
...
import pytest
from dotmac_kernel.messaging import platform_relay
...
@pytest.fixture()
def _cleanup(admin_session: Session):
    yield
    admin_session.execute(text("DELETE FROM platform_outbox_events"))
    admin_session.commit()


def test_claim_then_success(admin_session, dispatcher_sessionmaker, _cleanup) -> None:
    (eid,) = _seed(admin_session)
    s = dispatcher_sessionmaker()
    try:
        claimed = platform_relay.claim_platform_batch(
            s, worker_id="w1", policy=RelayPolicy(batch_size=10)
        )
        s.commit()
        mine = [c for c in claimed if c.id == eid]
        assert mine and mine[0].attempts == 0
        assert not hasattr(mine[0], "tenant_id")  # platform events have no tenant
        assert platform_relay.record_success(s, event_id=eid, worker_id="w1")
        s.commit()
    finally:
        s.close()
...
def test_failure_backs_off_then_dead_letters(
    admin_session, dispatcher_sessionmaker, _cleanup
) -> None:
    (eid,) = _seed(admin_session)
    policy = RelayPolicy(batch_size=10, max_attempts=3, base_backoff_seconds=1.0)
    s = dispatcher_sessionmaker()
    try:
        platform_relay.claim_platform_batch(s, worker_id="w1", policy=policy)
        s.commit()
        out = platform_relay.record_failure(
        ...
        admin_session.execute(
            text(
                "UPDATE platform_outbox_events SET available_at = now() WHERE id = :id"
            ),
            {"id": str(eid)},
        )
        admin_session.commit()
        platform_relay.claim_platform_batch(s, worker_id="w1", policy=policy)
        s.commit()
        out2 = platform_relay.record_failure(
...
def test_stale_lease_is_reclaimed(
    ...
    s = dispatcher_sessionmaker()
    try:
        platform_relay.claim_platform_batch(
            s, worker_id="w1", policy=RelayPolicy(batch_size=10)
        )
        s.commit()
        admin_session.execute(
            text(
                "UPDATE platform_outbox_events SET leased_at = now() - interval "
                "'1 hour' WHERE id = :id"
            ),
            {"id": str(eid)},
        )
        admin_session.commit()
        reclaimed = platform_relay.claim_platform_batch(
    ...
    leased_by = admin_session.execute(
        text("SELECT leased_by FROM platform_outbox_events WHERE id = :id"),
        {"id": str(eid)},
    ).scalar()
    assert leased_by == "w2"
...
def test_concurrent_workers_never_double_claim(
    ...
    ids = set(_seed(admin_session, n=6))
    a = dispatcher_sessionmaker()
    b = dispatcher_sessionmaker()
    try:
        a_claim = {
            c.id
            for c in platform_relay.claim_platform_batch(
