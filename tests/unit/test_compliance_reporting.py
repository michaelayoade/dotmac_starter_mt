...
import pytest
from dotmac_compliance_reporting import (
...
def db() -> Session:
    engine = create_engine(
        "sqlite:///:memory:",
        execution_options={"schema_translate_map": {"mod_compliance": None}},
    )
...
def test_pack_preserves_explicit_missing_sections_and_exact_digests(
    db: Session,
) -> None:
    tenant = _tenant(db)
    ...
    classification = publish_classification(
        ...
        section_codes=("complaints", "subscribers"),
...
def test_submission_and_acknowledgement_bind_the_exact_pack(db: Session) -> None:
    ...
    with pytest.raises(ComplianceRefused, match="digest"):
        submit_pack(
            ...
            tenant_id=tenant.id,
            pack_id=pack.id,
            submitted_pack_digest="f" * 64,
            submission_ref="filing:1",
            submitted_at=at,
        )
    submission = submit_pack(
        db,
        tenant_id=tenant.id,
        pack_id=pack.id,
        submitted_pack_digest=pack.pack_digest,
        submission_ref="filing:1",
        submitted_at=at,
    )
    acknowledgement = acknowledge_submission(
        db,
        tenant_id=tenant.id,
        submission_id=submission.id,
        command=AcknowledgementInput(
            "receipt:1", "accepted", at, "regulator:receipt:1"
        ),
    )
