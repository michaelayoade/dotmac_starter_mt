...
def db() -> Session:
    engine = create_engine(
        "sqlite:///:memory:",
        execution_options={"schema_translate_map": {"mod_supportaccess": None}},
    )
...
def _request(db: Session, *, key: str = "request:1"):
    return create_request(
        ...
        SupportRequestInput(
            ...
            "deployment:7",
            "supporter:9",
            ("subscriber.read", "session.inspect"),
        ),
...
def test_admission_binds_exact_approval_and_returns_no_credential(db: Session) -> None:
    ...
    with pytest.raises(AccessRefused, match="already admitted"):
        admit_request(
            ...
            admitted_at=datetime(2026, 8, 21, 8, 6, tzinfo=UTC),
            consent_evidence_ref="consent:2",
        )


def test_changed_content_break_glass_ceiling_and_missing_consent_fail_closed(
    db: Session,
) -> None:
    request = _request(db)
    admitted_at = datetime(2026, 8, 21, 8, 5, tzinfo=UTC)
...
def test_revoke_is_immediate_and_expiry_never_renews(db: Session) -> None:
    ...
    grant = admit_request(
        ...
        duration=timedelta(minutes=10),
        admitted_at=at,
        break_glass_reason="restore service",
    )
    revoke_grant(
        db,
        grant_id=grant.id,
    ...
    assert (
        db.scalar(
            select(SupportAccessGrant).where(SupportAccessGrant.id == expiring.id)
        )
        is expiring
    )
