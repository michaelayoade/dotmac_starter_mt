...
def _position(**overrides: object) -> ReceivableObservationV1:
    values: dict[str, object] = {
        "scope": SCOPE,
        "source_owner": "billing.receivables",
        "exposure_ref": "invoice:inv-1",
    ...
    values.update(overrides)
    return ReceivableObservationV1(**values)  # type: ignore[arg-type]
...
def test_current_position_requires_exact_non_negative_single_currency_money() -> None:
    ...
    with pytest.raises(ValueError, match="non-negative"):
        _position(collectible_receivable=Money.of(Decimal("-0.01"), NGN))
...
@pytest.mark.parametrize(
    ...
    [
        {"source_version": 0},
        {"due_date_status": "unknown_unverified", "due_at": AT},
        {
