...
from dotmac_remote_access import (
    AccessRefused,
...
def db() -> Session:
    engine = create_engine(
        "sqlite:///:memory:",
        execution_options={"schema_translate_map": {"mod_remoteaccess": None}},
    )
...
def test_admission_binds_exact_approval_and_emits_provider_neutral_intent(
    db: Session,
) -> None:
    tenant = _tenant(db)
    ...
    with pytest.raises(AccessRefused, match="digest"):
        admit_request(
            ...
            approved_request_digest="f" * 64,
            duration=timedelta(minutes=30),
            admitted_at=at,
        )
    grant, intent = admit_request(
        db,
        tenant_id=tenant.id,
        request_id=request.id,
        approval_evidence_ref="approval:1",
...
def test_expiry_and_revocation_fail_closed_and_observations_are_idempotent(
    db: Session,
) -> None:
    tenant = _tenant(db)
    at = datetime(2026, 8, 21, 8, tzinfo=UTC)
    first = create_request(
        ...
        tenant_id=tenant.id,
        command=RemoteAccessRequestInput(
            "a:1", "device:1", "repair", ("shell.read",), "op:1"
        ),
        requested_at=at,
    )
    grant, _ = admit_request(
        db,
        tenant_id=tenant.id,
        request_id=first.id,
        approval_evidence_ref="approval",
        approved_request_digest=first.request_digest,
    ...
    obs = record_observation(
        ...
        tenant_id=tenant.id,
        grant_id=grant.id,
        observation_key="receipt:1",
        action="revoke",
        outcome="applied",
        observed_at=at + timedelta(minutes=7),
        evidence_ref="network:receipt:1",
    )
    assert (
        record_observation(
            db,
    ...
    second = create_request(
        ...
        tenant_id=tenant.id,
        command=RemoteAccessRequestInput(
            "a:2", "device:2", "repair", ("shell.read",), "op:1"
        ),
        requested_at=at,
    )
    active, _ = admit_request(
        db,
        tenant_id=tenant.id,
        request_id=second.id,
        approval_evidence_ref="approval:2",
        approved_request_digest=second.request_digest,
        duration=timedelta(minutes=5),
        admitted_at=at,
    )
    intent = revoke_grant(
