...
def _request(
    *,
    ...
    expected_source_version: int | None = 7,
) -> TimerRequestV1:
    return TimerRequestV1(
        identity=identity or _identity(),
        due_at=due_at,
        recorded_at=recorded_at,
        output_event_type="collections.case.step_due.v1",
...
def test_timer_values_are_frozen_closed_and_carry_explicit_scope() -> None:
    ...
    assert request.expected_source_version == 7
    assert not hasattr(request, "__dict__")
    with pytest.raises(FrozenInstanceError):
        request.output_event_type = "changed"  # type: ignore[misc]
    with pytest.raises(ValueError, match="expected_source_version"):
        _request(expected_source_version=0)


def test_timer_port_requires_the_caller_owned_transaction() -> None:
    for method_name in ("schedule", "cancel", "accept_trigger", "current"):
        parameters = tuple(
            inspect.signature(getattr(CollectionsTimer, method_name)).parameters
        )
        assert parameters[:2] == ("self", "db")
    assert get_type_hints(CollectionsTimer.schedule)["db"] is Session


def test_shipped_fake_passes_the_reusable_timer_adapter_contract() -> None:
    assert_collections_timer_conformance(
        FakeCollectionsTimer(),
...
def test_fake_schedule_supersedes_and_stale_trigger_names_both_generations() -> None:
    ...
    first = timer.schedule(DB, _request())  # type: ignore[arg-type]
    ...
    assert first.expected_source_version == 7
    assert replacement.generation == 2
    assert replacement.timer_id != first.timer_id
    assert timer.current(DB, _identity()) == replacement  # type: ignore[arg-type]
    assert timer.accept_trigger(
        DB,  # type: ignore[arg-type]
        _trigger(first),
        accepted_at=AT + timedelta(days=1),
    ) == Stale(
...
def test_cancel_is_exact_and_has_four_distinct_typed_outcomes() -> None:
    timer = FakeCollectionsTimer()
    primary = timer.schedule(DB, _request())  # type: ignore[arg-type]
    sibling_identity = _identity(purpose="grace_expiry")
    sibling = timer.schedule(  # type: ignore[arg-type]
        DB, _request(identity=sibling_identity)
    )
    ...
    assert timer.current(DB, sibling_identity) == sibling  # type: ignore[arg-type]
    ...
    fired = fired_timer.schedule(DB, _request())  # type: ignore[arg-type]
    fired_timer.accept_trigger(  # type: ignore[arg-type]
        DB, _trigger(fired), accepted_at=fired.due_at
    )
...
def test_fake_acceptance_requires_a_caller_supplied_aware_instant() -> None:
    timer = FakeCollectionsTimer()
    handle = timer.schedule(DB, _request())  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="timezone-aware"):
