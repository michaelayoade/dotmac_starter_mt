...
def _publish_policy(db: Session, scope: Scope) -> uuid.UUID:
...
def _reader(position: ReceivableObservationV1) -> FakeReceivablesReader:
...
def _command(scope: Scope, ordinal: int) -> AssessCollectionExposureV1:
...
def test_catalog_contract_has_force_rls_and_exact_declared_tables(
    ...
    try:
        with engine.connect() as connection:
            ...
            assert set(actual) == set(TENANT_TABLES) | set(PLATFORM_TABLES)
            assert all(actual[table] == (True, True) for table in TENANT_TABLES)
            assert all(actual[table] == (False, False) for table in PLATFORM_TABLES)
    finally:
        engine.dispose()


def test_platform_role_reaches_only_the_platform_plane(
    scratch: tuple[str, str, str],
) -> None:
    _, app_url, platform_url = scratch
    app_engine = create_engine(app_url)
    try:
        with app_engine.connect() as connection, pytest.raises(DBAPIError):
            connection.execute(text("SELECT * FROM mod_coll.platform_collection_cases"))
    finally:
        app_engine.dispose()
    ...
    try:
        with Session(platform_engine) as session:
            ...
            assert len(session.scalars(select(PlatformCollectionCase)).all()) == 1
            with pytest.raises(DBAPIError):
                session.execute(select(CollectionCase))
    finally:
