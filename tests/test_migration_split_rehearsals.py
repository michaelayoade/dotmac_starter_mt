...
def test_rehearsal_1_fresh_empty_assembly(scratch_db: str) -> None:
    ...
    assert _versions(scratch_db) == {"0007_platform_identity"}
...
def test_rehearsal_2_fresh_reference_assembly(scratch_db: str) -> None:
    ...
    # Runtime version table: a001 subsumes the kernel head via `depends_on`
    # (a001 implies 0007 is applied), so alembic_version holds the assembly
    # head only while the assembly pins the latest kernel head. If the kernel
    # ever advances past what a001 depends on, both heads would appear.
    assert _versions(scratch_db) == {"a001_adopt_cfd"}
...
def _simulate_v08(url: str) -> None:
    ...
    _stamp(url, "kernel@head")  # drop the a001 record, keep the table
    assert _table_exists(url, "custom_field_definitions")
    assert _versions(url) == {"0007_platform_identity"}
...
def test_rehearsal_3_existing_v08_adoption(scratch_db: str) -> None:
    ...
    # (subsuming the kernel head via depends_on — see rehearsal 2).
    _upgrade(scratch_db, "heads")
    assert _versions(scratch_db) == {"a001_adopt_cfd"}
...
def test_rehearsal_6_runtime_rollback(scratch_db: str) -> None:
    ...
    # THE ROLLBACK PROCEDURE (new code): un-record a001 via stamp, table
    # preserved — NOT a downgrade (that would drop the table).
    _stamp(scratch_db, "kernel@head")
    assert _versions(scratch_db) == {"0007_platform_identity"}
    assert _table_exists(scratch_db, "custom_field_definitions")

    # Now the old v0.8 migrator succeeds: it sees only 0007, which it knows.
    _upgrade(scratch_db, "heads", version_locations=_kernel_only_locations())
    assert _versions(scratch_db) == {"0007_platform_identity"}
...
def test_rehearsal_7_expected_heads_per_lineage() -> None:
    ...
    assert kernel_head.revision == "0007_platform_identity"
