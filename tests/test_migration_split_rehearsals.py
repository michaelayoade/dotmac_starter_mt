...
def test_rehearsal_1_fresh_empty_assembly(scratch_db: str) -> None:
    ...
    # Kernel head advanced to 0008 (outbox/inbox); its tables exist, the
    # assembly's custom_field_definitions still does not.
    assert _table_exists(scratch_db, "outbox_events")
    assert _table_exists(scratch_db, "inbox_records")
    assert _versions(scratch_db) == {"0008_outbox_inbox"}
...
def test_rehearsal_2_fresh_reference_assembly(scratch_db: str) -> None:
    ...
    # it pins. The kernel has since advanced to 0008 (outbox/inbox), which a001
    ...
    assert _versions(scratch_db) == {"0008_outbox_inbox", "a001_adopt_cfd"}
...
def _simulate_v08(url: str) -> None:
    ...
    # kernel@head is now 0008 (the outbox/inbox tables also exist), but a001 is
    # un-recorded — the "table present, a001 not recorded" state adoption repairs.
    assert _versions(url) == {"0008_outbox_inbox"}
...
def test_rehearsal_3_existing_v08_adoption(scratch_db: str) -> None:
    ...
    # to 0008, so both lineage heads appear (see rehearsal 2).
    _upgrade(scratch_db, "heads")
    assert _versions(scratch_db) == {"0008_outbox_inbox", "a001_adopt_cfd"}
...
def test_rehearsal_7_expected_heads_per_lineage() -> None:
    ...
    assert kernel_head.revision == "0008_outbox_inbox"
