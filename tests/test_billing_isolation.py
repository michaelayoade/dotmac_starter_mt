...
def _money(amount: str, currency: str = "NGN") -> MoneyV1:
    return MoneyV1(Decimal(amount), currency, 2)
...
def test_settlement_replay_conflict_money_guards_and_rebuild_hash(scratch) -> None:
    ...
    with _session(platform_url, scope) as session:
        ...
        assert position_event.payload["exposure_ref"] == f"document:{invoice.id}"
        assert position_event.payload["due_date_basis"]["status"] == "verified"
...
def test_document_artifact_relation_enforces_exact_fact_content(scratch) -> None:
    ...
    with _session(platform_url, scope) as session:
        ...
        replacement_command = RecordDocumentArtifactV1(
            scope=scope,
            fact_id=fact.id,
            invoice_id=invoice.id,
            fact_version=fact.fact_version,
        ...
        replacement = record_document_artifact(
            session,
            scope=scope,
