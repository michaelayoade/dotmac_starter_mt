...
would let every surface be wrong together. The manifest is read through the
IMPORTED package attribute the release verifier itself resolves, so this gate
sees what composition sees rather than what a file happens to say.

## One guard, chosen over two others

Three versions of this check were written independently in one week, which is
itself evidence of how visible the drift had become. This is the one that
survives; the other two were deleted rather than left beside it, because a
weaker guard sitting next to a stronger one invites the strong one to be
skipped.

- `test_module_version_identity.py` returned silently when a manifest version
  was not found and perturbed a single manifest example in its sensitivity
  proof. Real drift landed independently in ALL THREE positions, so a proof
  that moves one of them does not establish the gate notices the other two.
- `test_package_version_sync.py` swept `packages/*` and read every surface with
  `ast`, which covers `dotmac-kernel` and `dotmac-ui` too — but it SKIPPED any
  package that did not expose a surface, which is the same hole in a different
  place, and it never imported anything, so it could not see what composition
  sees.
