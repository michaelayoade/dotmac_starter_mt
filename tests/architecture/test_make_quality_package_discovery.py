"""Every connector source root enrolls in quality checks by existing."""
...
def _make_sources(root: Path) -> set[str]:
    rule = "_print-connector-sources:\n\t@echo $(CONNECTOR_SOURCES)\n"
...
def test_quality_targets_consume_the_discovered_connector_roots() -> None:
    ...
    assert _make_sources(PROJECT_ROOT) == expected
