...
def test_the_migration_declares_the_same_prerequisites_as_the_manifest() -> None:
    ...
    assert all(migration["TENANT_REQUIRES"] == () for migration in migrations)
    assert all(migration["PLATFORM_REQUIRES"] == () for migration in migrations)
...
def test_the_floor_is_the_highest_prerequisite_release() -> None:
    """a100 publishes `dotmac_kernel.product_database_catalog`, which this
    module's new database_catalog= contribution imports, after the a68 audit
    prerequisite and the a66 ledger name."""
    manifest = tomllib.loads(
        (PACKAGE_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    )
    assert manifest["tool"]["poetry"]["dependencies"]["dotmac-kernel"] == ">=0.1.0a100"
