...
from dotmac_kernel.namespaces import AI_OPERATIONS_MIGRATION_OWNER, COMPLIANCE_REPORTING_MIGRATION_OWNER, MIGRATION_OWNER_LEDGER, REMOTE_ACCESS_MIGRATION_OWNER
...
CASES = (
    (remote_models, remote_module, REMOTE_ACCESS_MIGRATION_OWNER, "remote_access", "remoteaccess", "ra", "remote_access", "mod_remoteaccess", "ra_0001_remote_access.py"),
    (compliance_models, compliance_module, COMPLIANCE_REPORTING_MIGRATION_OWNER, "compliance_reporting", "compliance", "cr", "compliance_reporting", "mod_compliance", "cr_0001_compliance_reporting.py"),
    (ai_models, ai_module, AI_OPERATIONS_MIGRATION_OWNER, "ai_operations", "aiops", "ao", "ai_operations", "mod_aiops", "ao_0001_ai_operations.py"),
)


def test_each_module_owns_one_tenant_lineage_with_composite_identity() -> None:
    for models, module, owner, code, short_code, prefix, branch, schema, migration_name in CASES:
        ...
        for model in models.TENANT_MODELS:
            ...
            for constraint in model.__table__.foreign_key_constraints:
                if schema in {element.column.table.schema for element in constraint.elements}:
                    assert "tenant_id" in constraint.columns
        source = (Path(inspect.getfile(models)).parent / "migrations/versions" / migration_name).read_text(encoding="utf-8")
...
def test_modules_import_no_product_sibling_provider_or_transport() -> None:
    forbidden = {"app", "dotmac_sub", "dotmac_remote_access", "dotmac_compliance_reporting", "dotmac_ai_operations", "dotmac_integration", "httpx", "requests", "openai", "anthropic", "boto3"}
    roots_by_module = {
...
def test_remote_and_ai_tables_structurally_hold_no_secret_or_endpoint() -> None:
    forbidden = {"credential", "password", "secret", "api_key", "endpoint", "access_token", "private_key"}
    columns = {column.name for models in (remote_models, ai_models) for model in models.TENANT_MODELS for column in model.__table__.columns}
