...
#
# Subscriptions is the current occupant: its manifest vocabularies and namespace
# allocation land together in a71. Older releasable modules have all outlived
# their own allocation floors and therefore live in the capability-raised map.
# `test_every_releasable_module_has_a_floor_rule` keeps both maps watched: a
# module may not be absent from both once release-enrolled.
...
def test_every_releasable_module_has_a_floor_rule() -> None:
    ...
    A parametrized floor map can become empty and report green having asserted
    nothing. That is only tolerable because a module cannot fall out of both
    maps: this reads
