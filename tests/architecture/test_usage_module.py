"""Structural canaries for normalized usage ownership."""
...
from dotmac_kernel.namespaces import MIGRATION_OWNER_LEDGER, USAGE_MIGRATION_OWNER
from dotmac_usage import models, service
from dotmac_usage.manifest import module

ROOT = Path(inspect.getfile(service)).parent
MIGRATION = ROOT / "migrations/versions/us_0001_usage_facts.py"
REPO_ROOT = Path(__file__).resolve().parents[2]


def test_manifest_and_secure_plane_are_exact() -> None:
    assert USAGE_MIGRATION_OWNER in MIGRATION_OWNER_LEDGER
    assert (
        module.code,
        module.short_code,
        module.migration_prefix,
        module.db_schema,
    ) == ("usage", "usage", "us", "mod_usage")
...
def test_usage_is_normalized_and_not_raw_aaa_or_rating() -> None:
    forbidden = {
        "nas_ip",
        "calling_station_id",
        "framed_ip_address",
...
def test_root_migration_declares_the_whole_forced_rls_plane() -> None:
    ...
    assert ast.literal_eval(assigned["revision"]) == "us_0001_usage_facts"
    assert ast.literal_eval(assigned["down_revision"]) is None
    assert ast.literal_eval(assigned["branch_labels"]) == ("usage",)
    assert ast.literal_eval(assigned["REQUIRES"]) == tuple(module.requires)
    for table in module.tables:
        qualified = f"mod_usage.{table}"
