...
_INTERNAL_RELATIONS = (
    ("rated_obligations", ("billing_account_id",), "billing_accounts", ("id",)),
    (
        "rated_obligations",
    ...
    ("documents", ("billing_account_id",), "billing_accounts", ("id",)),
    (
        "documents",
    ...
    ("document_events", ("document_id",), "documents", ("id",)),
    (
    ...
    ("posting_groups", ("billing_account_id",), "billing_accounts", ("id",)),
    (
        "posting_groups",
    ...
    ("applied_tax_snapshots", ("document_id",), "documents", ("id",)),
    (
    ...
    ("applied_fx_snapshots", ("document_id",), "documents", ("id",)),
    (
    ...
    ("invoice_document_facts", ("document_id",), "documents", ("id",)),
    (
        "document_artifacts",
        ("document_fact_id", "document_id", "fact_version"),
        "invoice_document_facts",
        ("id", "document_id", "fact_version"),
    ),
    (
        "document_artifacts",
        ("superseded_by_artifact_id", "document_fact_id", "media_type"),
        "document_artifacts",
        ("id", "document_fact_id", "media_type"),
    ),
    ("accounting_facts", ("posting_group_id",), "posting_groups", ("id",)),
    ("accounting_facts", ("reverses_fact_id",), "accounting_facts", ("id",)),
    (
...
def _expected_internal_foreign_keys() -> set[tuple[object, ...]]:
    expected: set[tuple[object, ...]] = set()
    for source, local, target, remote in _INTERNAL_RELATIONS:
        expected.add(
            (
                source,
                ("tenant_id", *local),
...
def _model_internal_foreign_keys() -> set[tuple[object, ...]]:
...
def test_every_billing_reference_is_database_enforced_on_both_planes() -> None:
    """No service convention may be the only thing preventing dangling facts."""

    assert _model_internal_foreign_keys() == _expected_internal_foreign_keys()


def test_internal_reference_guard_is_sensitive_to_one_missing_edge() -> None:
    expected = _expected_internal_foreign_keys()
    missing = next(iter(expected))

    assert (expected - {missing}) != expected
...
def test_public_contracts_freeze_only_the_agreed_v1_shapes() -> None:
    assert not hasattr(contracts, "OBLIGATION_OUTPUT_CONTRACT")
    assert contracts.OBLIGATION_ACCEPT_CONTRACT == "billing.obligation.accept.v1"
    ...
    assert contracts.RECEIVABLE_EXPOSURE_CONTRACT == "billing.receivable.exposure.v1"
    assert contracts.ARTIFACT_COMMAND_CONTRACT == "billing.document.artifact.record.v1"
    assert contracts.ARTIFACT_REPAIR_CONTRACT == "billing.document.artifact.repair.v1"
    ...
    repair_fields = {
        field.name for field in dataclasses.fields(contracts.RepairDocumentArtifactV1)
    }
...
def test_runtime_version_and_unreleased_kernel_floor_agree() -> None:
    ...
    assert package["tool"]["poetry"]["dependencies"]["dotmac-kernel"] == ">=0.1.0a89"
