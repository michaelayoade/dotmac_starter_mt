...
def test_policy_rows_are_cross_tenant_isolated(migrated: tuple[str, str]) -> None:
    ...
    with admin.begin() as connection:
        for index, tenant in enumerate(tenants):
            ...
            connection.execute(
                text(
                    "INSERT INTO mod_svc_access.service_access_inputs "
            ...
            connection.execute(
                text(
                    "INSERT INTO mod_svc_access.desired_access_decisions "
    ...
    try:
        for tenant in tenants:
            with app.begin() as connection:
                ...
                for table in TABLES:
                    query = f"SELECT tenant_id FROM mod_svc_access.{table}"  # noqa: S608
