...
VERSIONS = (
    ROOT
    / "packages/dotmac-support-access/src/dotmac_support_access/migrations/versions"
)
...
def test_support_access_platform_catalog_and_privileges(
    ...
    with engine.connect() as conn:
        ...
        secretish = conn.execute(
            text(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_schema = 'mod_supportaccess' AND column_name IN "
        ...
        for table in TABLES:
            enabled, forced = conn.execute(
                ...
                {"table": f"mod_supportaccess.{table}"},
            ).one()
            ...
            assert any(
                conn.scalar(
                    ...
                    {"table": f"mod_supportaccess.{table}", "privilege": p},
                )
    ...
    with platform.connect() as conn:
        assert (
            conn.scalar(
                text("SELECT count(*) FROM mod_supportaccess.support_access_requests")
            )
            == 0
        )
    platform.dispose()
