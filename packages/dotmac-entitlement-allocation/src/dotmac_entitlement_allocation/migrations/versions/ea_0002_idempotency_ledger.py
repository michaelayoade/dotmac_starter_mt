...
`dotmac_entitlement_allocation.service.stage_allocation` delegates
at-most-once execution to `dotmac_kernel.idempotency.execute_once_platform`
(hard rule 21, ADR-0014: the ledger has exactly ONE owner and this module is
not it). That call writes `public.platform_idempotency_records` at REQUEST
time, and `ea_0001` — the module's only other revision — creates nothing of
the sort.
...
`UndefinedTable` the first time an activation is staged. Nothing could have
...
`-v0.1.0a3` and `-v0.1.0a4` — four published tags, all four holding the same
bytes. Those bytes have run in databases this repository does not own, so they
are history: editing them would make the checked-in lineage disagree with what
...
author reaching for the easier edit; this change is what extends that guard to
cover this distribution's tags.)
...
`dotmac-integration` took this route in `ig_0007` (#204), for this reason.

## Why a migration is the right place at all

Deploy is the LAST moment at which a missing ledger is a failed migration
rather than a failed staging call in production. `require_prerequisites`
inspects the live catalog — the column contract, the unique key per plane, the
...
Revision ID: ea_0002_idempotency_ledger
Revises: ea_0001_allocations
...
revision = "ea_0002_idempotency_ledger"
down_revision = "ea_0001_allocations"
branch_labels = None

# COMMON, and the three lists are written out in full so the reader sees the
# decision rather than an omission.
#
# The call site is platform-only — `execute_once_platform` is the sole
# at-most-once entry point in this package and `execute_once` appears nowhere
# — which LOOKS like a `platform_requires`, and is not. This module owns
# exactly one plane (`tables` is empty, `supported_plane_sets` unset), so the
# platform plane installs atomically and no selection exists under which the
# ledger stops being needed; a plane-specific list would condition on
# something that cannot vary. It would also be unresolvable:
...
# installed — `validate_module_plane_selections` refuses one outright.
...
def downgrade() -> None:
    ...
    Not `raise NotImplementedError`. Refusing here would make the whole `ea`
    lineage undowngradable past this point over a revision that changed
    nothing, which is a worse lie than an empty body — and the live canaries
