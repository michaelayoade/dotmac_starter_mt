...
Pending Billing `0.1.0a1` deliberately publishes no coverage contract. That is
an unresolved adoption boundary, not permission for an assembly adapter to make
the decision: before Sub cutover, Billing must publish an owner-decided
obligation-resolution fact that can be translated mechanically into
`RecordCoverageResolutionCommand`, or a different named owner must supply it.
