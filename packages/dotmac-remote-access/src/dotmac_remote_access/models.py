...
from sqlalchemy import JSON, DateTime, ForeignKey, ForeignKeyConstraint, String, Text, UniqueConstraint, Uuid
...
class RemoteAccessRequest(Base, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "id", name="uq_remote_requests_tenant_id_id"), UniqueConstraint("tenant_id", "request_key", name="uq_remote_requests_key"), schema_table_args(SCHEMA))
...
class RemoteAccessGrant(Base, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "id", name="uq_remote_grants_tenant_id_id"), UniqueConstraint("tenant_id", "request_id", name="uq_remote_grants_request"), ForeignKeyConstraint(["tenant_id", "request_id"], [f"{SCHEMA}.remote_access_requests.tenant_id", f"{SCHEMA}.remote_access_requests.id"], name="fk_remote_grants_request", ondelete="RESTRICT"), schema_table_args(SCHEMA))
    id: Mapped[UUID] = uuid_pk()
    tenant_id: Mapped[UUID] = tenant_id_column()
...
class RemoteAccessObservation(Base):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "id", name="uq_remote_observations_tenant_id_id"), UniqueConstraint("tenant_id", "observation_key", name="uq_remote_observations_key"), ForeignKeyConstraint(["tenant_id", "grant_id"], [f"{SCHEMA}.remote_access_grants.tenant_id", f"{SCHEMA}.remote_access_grants.id"], name="fk_remote_observations_grant", ondelete="RESTRICT"), schema_table_args(SCHEMA))
    id: Mapped[UUID] = uuid_pk()
    tenant_id: Mapped[UUID] = tenant_id_column()
...
__all__ = ["RemoteAccessGrant", "RemoteAccessObservation", "RemoteAccessRequest", "SCHEMA", "TENANT_MODELS", "TENANT_TABLES"]
