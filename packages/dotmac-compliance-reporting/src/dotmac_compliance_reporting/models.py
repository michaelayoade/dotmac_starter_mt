...
class ReportingObligation(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("tenant_id", "code", name="uq_reporting_obligations_code"),
...
class ClassificationRevision(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            [
                f"{SCHEMA}.reporting_obligations.tenant_id",
                f"{SCHEMA}.reporting_obligations.id",
            ],
            ondelete="RESTRICT",
        ),
        schema_table_args(SCHEMA),
...
class EvidencePack(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "id", name="uq_evidence_packs_tenant_id_id"),
        UniqueConstraint(
            "tenant_id",
            "obligation_id",
            "period_start",
        ...
        ForeignKeyConstraint(
            ...
            [
                f"{SCHEMA}.classification_revisions.tenant_id",
                f"{SCHEMA}.classification_revisions.id",
            ],
            ondelete="RESTRICT",
        ),
        schema_table_args(SCHEMA),
    ...
    tenant_id: Mapped[UUID] = tenant_id_column()
    obligation_id: Mapped[UUID] = mapped_column(Uuid(), nullable=False)
    classification_revision_id: Mapped[UUID] = mapped_column(Uuid(), nullable=False)
    ...
    sections: Mapped[list[EvidenceSection]] = relationship(
        lambda: EvidenceSection, order_by=lambda: EvidenceSection.section_code
    )
...
class EvidenceSection(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "id", name="uq_evidence_sections_tenant_id_id"),
        UniqueConstraint(
            "tenant_id",
            "pack_id",
            "section_code",
        ...
        ForeignKeyConstraint(
            ["tenant_id", "pack_id"],
            [f"{SCHEMA}.evidence_packs.tenant_id", f"{SCHEMA}.evidence_packs.id"],
            ondelete="CASCADE",
        ),
        schema_table_args(SCHEMA),
    )
...
class FilingSubmission(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "id", name="uq_filing_submissions_tenant_id_id"),
    ...
    tenant_id: Mapped[UUID] = tenant_id_column()
    pack_id: Mapped[UUID] = mapped_column(Uuid(), nullable=False)
    submitted_pack_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    submission_ref: Mapped[str] = mapped_column(String(240), nullable=False)
...
class RegulatorAcknowledgement(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            [
                f"{SCHEMA}.filing_submissions.tenant_id",
                f"{SCHEMA}.filing_submissions.id",
            ],
            ondelete="RESTRICT",
        ),
        schema_table_args(SCHEMA),
    ...
    acknowledgement_key: Mapped[str] = mapped_column(String(200), nullable=False)
