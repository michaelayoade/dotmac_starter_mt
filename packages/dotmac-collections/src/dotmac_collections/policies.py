...
EffectScope = Literal["obligation", "service", "contract", "subject"]
...
class PolicyStepDraftV1:
    ...
    def __post_init__(self) -> None:
        ...
        if self.request_kind == "action":
            ...
            if self.effect_scope is None:
                raise ValueError("action step requires effect_scope")
            if self.effect_scope not in (
                "obligation",
                "service",
                "contract",
                "subject",
            ):
                raise ValueError("action effect_scope is unsupported")
            if self.purpose_code is not None:
                raise ValueError("action step cannot carry purpose_code")
        elif self.action_code is not None:
            raise ValueError("notice step cannot carry action_code")
        elif self.effect_scope is not None:
            raise ValueError("notice step cannot carry effect_scope")
        elif self.purpose_code is None:
            raise ValueError("notice step requires purpose_code")
        else:
            require_text("purpose_code", self.purpose_code)
        for retry_offset in self.retry_offsets:
            if retry_offset < timedelta(0):
                raise ValueError("retry offsets must be non-negative")
            ...
            "steps": [
                {
                    ...
                    "retry_offsets": [
                        _duration_value(offset) for offset in step.retry_offsets
                    ],
                }
...
PolicyDecision = StepDue | StepWaiting | AnchorUnavailable | LadderComplete
