"""Canonical, server-owned request state for list and queue surfaces.

Products declare searchable, filterable and sortable capabilities with a
``ListDefinition``. Adapters pass raw request values to that owner and render
the resulting ``ListQuery`` / ``PageMeta`` instead of rebuilding query-string,
pagination or canonicalization rules in templates or JavaScript.

The contract is persistence- and framework-neutral. Product services still own
the actual query, authorization, counts, row projection, bulk effects and
exports.
"""
...
class ListFieldDefinition:
    """One declared search, filter or sort capability."""
...
class ListDefinition:
    """Authoritative request capabilities and defaults for one list resource."""
...
def request_needs_canonicalization(
    ...
    *,
    ...
    """Whether raw request state differs from the owner's normalized state."""
...
class PageMeta:
    ...
    def navigation(self) -> tuple[int | None, ...]:
        """Compact page-number sequence; ``None`` is one ellipsis."""
