...
| `dotmac_kernel.api_documentation` | `ApiDocumentationPolicy`, `ApiDocumentationPolicyError`, `ApiDocumentationPolicyViolation`, `DocumentationPlane`, `DocumentationExposure`, `DocumentationRoute`, `POLICIES`, `classify_environment`, `api_documentation_policy`, `environment_api_documentation_policy`, `documentation_arguments`, `documentation_routes`, `audit_api_documentation`, `mount_bearer_protected_document`, and the path/plane constants (who may read an assembly's API documentation. `ProductAssemblySpec.api_documentation` is REQUIRED — `create_app` refuses to build without it. Suppression happens at FastAPI construction; resolution fails closed to production) |
...
| `dotmac_kernel.machine_auth` | `MachinePrincipal`, `authenticate_machine`, `require_machine_scope`, `hash_machine_key`, `API_KEY_HEADER`, `MACHINE_KEY_SECRET_NAME`, `MachineKeyUnavailableError` — an `X-Api-Key` principal that is not a person; empty scopes authorize nothing and there is no wildcard, prefix or substring match. `MachinePrincipal.application` names the source application, and a credential with no attribution does not authenticate |
| `dotmac_kernel.machine_models` | `MachineCredential` |
| `dotmac_kernel.machine_rotation` | `issue_credential`, `begin_rotation`, `complete_rotation`, `cancel_rotation`, `revoke_credential`, `RotationStateError` — the WRITE side, kept out of `machine_auth` so that module stays one SELECT. A rotation is an explicit window: both secrets authenticate until `complete_rotation`, and nothing retires the outgoing one on a timer |
| `dotmac_kernel.source_applications` | `SourceApplicationRegistry`, `install_source_applications`, `active_source_applications`, `install_host_application`, `active_host_application`, `clear_host_application`, `host_application_or_none`, `validate_source_application`, `SOURCE_APPLICATION_MAX_LENGTH`, `SourceApplicationsNotInstalledError`, `HostApplicationNotInstalledError`, `InvalidSourceApplicationError`, `UndeclaredSourceApplicationError` — which APPLICATION issued a command, presented a credential, or caused an audit row. Open registered strings, never an enum; exact membership, no wildcard entry |
...
| `dotmac_kernel.facet_principal` | `FacetPrincipal`, `FacetPrincipalError`, `FacetPrincipalUnavailableError`, `FacetPrincipalPlaneMismatchError`, `facet_principal`, `require_facet_principal` (the principal a browser facet ALREADY authenticated, projected for module-owned surfaces; a projection, never an authenticator — it reads request-scoped state only, refuses an absent principal, and refuses one from a different `BrowserSecurityPlane`. `record_facet_principal` is kernel-internal composition, not an extension point) |
...
| `dotmac_kernel.product_database_catalog` | `PRODUCT_DATABASE_CATALOG_SCHEMA`, typed owner/plane/relation/column/type declarations, `ModuleDatabaseCatalogContributionV1`, `HostDatabaseCatalogFragmentV1`, `ProductDatabaseCatalogSnapshot`, and its strict parser/errors (canonical release-bound database structure; also top-level) |
| `dotmac_kernel.database_catalog_comparator` | typed PostgreSQL tables/columns observation, pure module/product comparison, digest-bound verification, and its stable observation/comparator IDs (also top-level) |
...
| `dotmac_kernel.semantic_encoding` | `encode`, `encode_fields`, `encode_ordered`, `encode_unordered`, `digest_of`, `ABSENT`, `CANONICAL_ALGORITHM`, `CanonicalEncodingError`, `canonical_decimal`, `canonical_instant` (the length-prefixed, self-describing encoding every `cv1:` digest is taken over. `encode_ordered` accepts VALUES and therefore frames a `bytes` value with the same `x` tag as `encode`; already-encoded frame composition is kernel-internal. The ENCODING is the contract: a change to produced bytes changes every digest computed from them, so it moves only with a called-out break) |
