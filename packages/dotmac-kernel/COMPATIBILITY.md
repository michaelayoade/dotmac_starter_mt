...
| `dotmac_kernel.semantic_encoding` | `encode`, `encode_fields`, `encode_ordered`, `encode_unordered`, `digest_of`, `ABSENT`, `CANONICAL_ALGORITHM` (the length-prefixed, self-describing encoding every `cv1:` digest is taken over. The ENCODING is the contract: a change to it changes every digest computed from it, so it moves only with a called-out break) |
