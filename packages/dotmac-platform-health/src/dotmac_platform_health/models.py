...
class HealthComponent(Base, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint("code", name="uq_health_components_code"), schema_table_args(SCHEMA))
...
class HealthObservation(Base):
    ...
    __table_args__ = (
        UniqueConstraint("source_ref", "observation_key", name="uq_health_observations_source_key"),
        schema_table_args(SCHEMA),
    )

    id: Mapped[UUID] = uuid_pk()
    component_id: Mapped[UUID] = mapped_column(ForeignKey(f"{SCHEMA}.health_components.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class HealthProjection(Base, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint("component_id", name="uq_health_projections_component"), schema_table_args(SCHEMA))

    id: Mapped[UUID] = uuid_pk()
    component_id: Mapped[UUID] = mapped_column(ForeignKey(f"{SCHEMA}.health_components.id", ondelete="CASCADE"), nullable=False)
    observation_id: Mapped[UUID] = mapped_column(ForeignKey(f"{SCHEMA}.health_observations.id", ondelete="RESTRICT"), nullable=False)
    state: Mapped[str] = mapped_column(String(24), nullable=False)
    observed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
__all__ = ["HealthComponent", "HealthObservation", "HealthProjection", "PLATFORM_MODELS", "PLATFORM_TABLES", "SCHEMA"]
