...
The IDENTITY is the binding; the KEY is the bearer credential addressing it.
That separation is what makes rotation cheap — the address a third party holds
changes while every receipt, every delivery and every foreign key stays put.
...
directly instead — at the COLUMN grain, which is the grain the assumption is
about.

No new table: the manifest's `platform_tables` stays at seven, and this module
stays platform-plane-only (ADR-0023). A column carries no plane of its own.
...
_INDEX = "uq_capability_bindings_ingress_endpoint"
...
def upgrade() -> None:
    ...
    # can never be shared. Two bindings sharing one key would route one
    # provider's traffic into another operator's configuration.
    op.create_index(
        _INDEX,
        _BINDINGS,
...
def downgrade() -> None:
    op.drop_index(_INDEX, table_name=_BINDINGS, schema=_SCHEMA)
