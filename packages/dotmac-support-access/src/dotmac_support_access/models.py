...
class SupportAccessRequest(Base, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint("request_key", name="uq_support_requests_key"), schema_table_args(SCHEMA))
...
class SupportAccessGrant(Base, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint("request_id", name="uq_support_grants_request"), schema_table_args(SCHEMA))

    id: Mapped[UUID] = uuid_pk()
    request_id: Mapped[UUID] = mapped_column(ForeignKey(f"{SCHEMA}.support_access_requests.id", ondelete="RESTRICT"), nullable=False)
    case_ref: Mapped[str] = mapped_column(String(200), nullable=False)
    purpose: Mapped[str] = mapped_column(Text, nullable=False)
    target_ref: Mapped[str] = mapped_column(String(200), nullable=False)
    requester_ref: Mapped[str] = mapped_column(String(200), nullable=False)
    ...
    def descriptor(self) -> FiniteGrantDescriptor:
        return FiniteGrantDescriptor(self.id, self.case_ref, self.purpose, self.target_ref, self.requester_ref, tuple(self.capabilities), self.mode, self.issued_at, self.expires_at)


class SupportAccessEvent(Base):
    __tablename__ = "support_access_events"
    __table_args__ = (UniqueConstraint("request_id", "sequence", name="uq_support_events_sequence"), schema_table_args(SCHEMA))

    id: Mapped[UUID] = uuid_pk()
    request_id: Mapped[UUID] = mapped_column(ForeignKey(f"{SCHEMA}.support_access_requests.id", ondelete="RESTRICT"), nullable=False)
    grant_id: Mapped[UUID | None] = mapped_column(ForeignKey(f"{SCHEMA}.support_access_grants.id", ondelete="RESTRICT"))
...
__all__ = ["PLATFORM_MODELS", "PLATFORM_TABLES", "SCHEMA", "SupportAccessEvent", "SupportAccessGrant", "SupportAccessRequest"]
