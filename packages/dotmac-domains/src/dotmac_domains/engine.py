...
_CONFIRMATION_TRANSITIONS: dict[
    ...
    (DomainLifecycleState.EXPIRED, "deleted"): DomainLifecycleState.RELEASED,
    (DomainLifecycleState.REDEMPTION, "deleted"): DomainLifecycleState.RELEASED,
}
...
def decide_lifecycle_transition(value: LifecycleInput) -> LifecycleDecision:
    ...
    if value.current_state in {
        DomainLifecycleState.RELEASED,
        DomainLifecycleState.REGISTRATION_FAILED,
    }:
...
def derive_drift(
    *,
    ...
    desired_dns_nameservers: tuple[str, ...],
    observed_dns_nameservers: tuple[str, ...],
    desired_dns_recordsets_digest: str | None,
    observed_dns_recordsets_digest: str | None,
) -> DomainDrift:
    reasons: list[str] = []
    expiry_disagrees = (
    ...
    dns_nameservers_disagree = bool(desired_dns_nameservers) and tuple(
        desired_dns_nameservers
    ) != tuple(observed_dns_nameservers)
    if dns_nameservers_disagree:
        reasons.append("dns_nameservers_disagree")
    dns_recordsets_disagree = (
        desired_dns_recordsets_digest is not None
        and observed_dns_recordsets_digest is not None
        and desired_dns_recordsets_digest != observed_dns_recordsets_digest
    )
    if dns_recordsets_disagree:
        reasons.append("dns_recordsets_disagree")
    return DomainDrift(
        ...
        dns_nameservers_disagree=dns_nameservers_disagree,
        dns_recordsets_disagree=dns_recordsets_disagree,
