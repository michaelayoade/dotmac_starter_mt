...
exist before a local service does, so their optional correlation id is
deliberately not a foreign key; canonical-name reconciliation joins it later.
"""
...
def upgrade() -> None:
    ...
    op.create_index(
        ...
        "domain_holds",
        ["tenant_id", "domain_service_id", "hold_code"],
        unique=True,
        schema=_SCHEMA,
    ...
    op.execute(
        ...
        RETURNS trigger AS $$
        BEGIN
            RAISE EXCEPTION 'domain command, outcome, observation and intent evidence is immutable'
    ...
    op.execute(
        "GRANT SELECT, INSERT, UPDATE, DELETE ON mod_domains.domain_services TO app_user;"
    )
