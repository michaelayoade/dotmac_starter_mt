...
class DomainService(Base):
    ...
    registrar_binding_ref: Mapped[str | None] = mapped_column(String(255))
    dns_binding_ref: Mapped[str | None] = mapped_column(String(255))
...
class DomainObservation(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    tenant_id: Mapped[uuid.UUID] = _tenant_id()
    # Nullable so a callback may predate local correlation; when populated the
    # composite FK proves it names a service in the same tenant.
...
class DNSObservation(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "id", name="uq_dns_observations_tenant_id_id"),
        UniqueConstraint(
            "tenant_id",
            "capability_binding_ref",
            "provider_event_id",
    ...
    provider_event_id: Mapped[str] = mapped_column(String(255), nullable=False)
    observed_nameservers: Mapped[list[str]] = mapped_column(JSON, nullable=False)
    observed_recordsets: Mapped[list[dict[str, object]]] = mapped_column(
        JSON, nullable=False
    )
    observed_recordsets_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    source_mode: Mapped[str] = mapped_column(String(16), nullable=False)
    payload_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    observed_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
