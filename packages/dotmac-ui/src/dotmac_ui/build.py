"""Generate the published assets from the token vocabulary.

INTERNAL — not part of the consumer contract (`dotmac_ui.INTERNAL_MODULES`). A
consumer references the *output* (`dotmac_ui.assets`); only this repository runs
the generator, via `make ui-build` or `python -m dotmac_ui.build`.

**Why a pure-Python generator and not Tailwind.** ADR-0006 D3 says the package
MAY build its own assets with Tailwind v4; it does not say it must. What U1
publishes is a token layer plus one base rule — there is nothing to compile,
scan, or purge, so introducing npm here would buy a dependency and a
non-hermetic build step in exchange for nothing. The decision that matters to
consumers is the *boundary*, not the tool behind it: when a later slice adds the
component library and does want Tailwind v4 to build it, that changes this file
and changes nothing a consumer does, which is precisely the property D3 is
protecting.

**The output is committed.** The generated stylesheet and manifest are tracked
in git, not gitignored build artifacts, because they ARE the published contract
— a reviewer must see a token change as a diff in the CSS, and an air-gapped
consumer must get working assets from a checkout with no build step.
`test_committed_stylesheet_matches_a_fresh_build` fails if the two drift, so the
committed copy cannot quietly become a fork of its own source.

Generation is deterministic: same tokens in, byte-identical file out.
"""

from __future__ import annotations

import argparse
import json
import sys
from hashlib import sha256
from pathlib import Path

from dotmac_ui.assets import (
    BEHAVIOR_SCRIPT_RELPATH,
    MANIFEST_RELPATH,
    STYLESHEET_RELPATH,
    TAILWIND_PRESET_RELPATH,
    static_dir,
)
from dotmac_ui.contract import (
    ACCESSIBILITY_TARGET,
    DARK_THEME_SELECTORS,
    TOKEN_PREFIX,
    UI_CONTRACT_VERSION,
)
from dotmac_ui.tokens import (
    REDUCED_MOTION_DURATION,
    TOKENS,
    channel_variable,
    colour_tokens,
    declarations,
    iter_categories,
    resolve_color,
    token,
    tokens_in,
)

_BEHAVIOR_SOURCE_PATH = Path(__file__).with_name("_behavior_source.js")


def _channels(hex_colour: str) -> str:
    """`#3b82f6` -> `59 130 246`, the space-separated form `rgb()` wants.

    Tailwind synthesises `bg-brand-500/50` as `rgb(<channels> / 0.5)`, so it
    needs the components separately. A variable holding a complete colour cannot
    take an alpha modifier at all — the utility silently renders opaque.
    """
    value = hex_colour.lstrip("#")
    if len(value) == 3:
        value = "".join(ch * 2 for ch in value)
    if len(value) != 6:
        raise ValueError(f"expected a 6-digit hex colour, got {hex_colour!r}")
    return " ".join(str(int(value[i : i + 2], 16)) for i in (0, 2, 4))


def _channel_declarations(mode: str) -> list[tuple[str, str]]:
    """`(--dmui-…-rgb, "R G B")` for every colour token, resolved for `mode`."""
    return [
        (
            channel_variable(design_token.name),
            _channels(resolve_color(design_token.name, mode)),
        )
        for design_token in colour_tokens()
    ]


_INDENT = "  "


def _banner(package_version: str) -> str:
    return "\n".join(
        (
            "/*!",
            " * dotmac-ui — the DotMac shared UI design system.",
            f" * UI contract version: {UI_CONTRACT_VERSION}",
            f" * Package version:     {package_version}",
            f" * Accessibility:       {ACCESSIBILITY_TARGET}",
            " *",
            " * GENERATED FILE — do not edit by hand. The source of truth is",
            " * dotmac_ui/tokens.py; rebuild with `make ui-build`.",
            " *",
            " * This file is self-contained: no @import, no remote origin, no",
            " * web font, no CDN. It needs no preprocessor and no particular",
            " * Tailwind major — see dotmac_ui/assets.py.",
            " */",
        )
    )


def _root_block() -> str:
    lines = [":root {"]
    first = True
    for category, members in iter_categories():
        if not first:
            lines.append("")
        first = False
        lines.append(f"{_INDENT}/* {category} */")
        for design_token in members:
            lines.append(f"{_INDENT}{design_token.variable}: {design_token.value};")
    lines.append("")
    lines.append(f"{_INDENT}/* channel forms, for alpha modifiers — see _channels */")
    for variable, value in _channel_declarations("light"):
        lines.append(f"{_INDENT}{variable}: {value};")
    lines.append("}")
    return "\n".join(lines)


def _dark_block() -> str:
    selector = ",\n".join(DARK_THEME_SELECTORS)
    lines = [
        "/* Dark mode. Only the mode-DEPENDENT roles are restated: the ramps are",
        " * identical in both modes, so a runtime brand override re-declares them",
        " * once and every role that points at them follows. `.dark` is honoured",
        " * alongside the package's own attribute so a host already using",
        " * Tailwind's class strategy needs no template change. */",
        f"{selector} {{",
    ]
    for variable, value in declarations("dark"):
        lines.append(f"{_INDENT}{variable}: {value};")
    lines.append("")
    lines.append(f"{_INDENT}/* channel forms restated: an alpha modifier must")
    lines.append(f"{_INDENT} * darken with its colour, not stay on the light one. */")
    for variable, value in _channel_declarations("dark"):
        lines.append(f"{_INDENT}{variable}: {value};")
    lines.append("}")
    return "\n".join(lines)


def _reduced_motion_block() -> str:
    lines = [
        "/* WCAG 2.2 SC 2.3.3 — a user who asks for reduced motion gets it from",
        f" * the token layer, not from each component. {REDUCED_MOTION_DURATION}"
        " rather than 0s so",
        " * transitionend/animationend handlers still fire and no interaction",
        " * that waits on one can hang. */",
        "@media (prefers-reduced-motion: reduce) {",
        f"{_INDENT}:root {{",
    ]
    for design_token in tokens_in("motion"):
        if design_token.name.startswith("duration-"):
            lines.append(
                f"{_INDENT * 2}{design_token.variable}: {REDUCED_MOTION_DURATION};"
            )
    lines.append(f"{_INDENT}}}")
    lines.append("}")
    return "\n".join(lines)


def _base_block() -> str:
    return "\n".join(
        (
            "/* The base rule this contract ships. WCAG 2.2 SC 2.4.11 (focus",
            " * appearance) is a token-layer promise, so the indicator itself is",
            " * defined here rather than left to each consumer to re-derive.",
            " * Deliberately unlayered and at (0,1,0) specificity: an @layer would",
            " * lose to every unlayered rule in the host stylesheet, which would",
            " * silently give the promise away. */",
            ":focus-visible {",
            f"{_INDENT}outline: var({TOKEN_PREFIX}focus-ring-width) solid "
            f"var({TOKEN_PREFIX}focus-ring-color);",
            f"{_INDENT}outline-offset: var({TOKEN_PREFIX}focus-ring-offset);",
            "}",
        )
    )


def _components_block() -> str:
    """Styling for the published Jinja components (`dotmac_ui.components`).

    Every declaration resolves through a token, so a consumer that overrides the
    `--dmui-*` ramps re-themes the components for free and never has to restyle
    this markup. Selectors are single-class (0,1,0) and unlayered for the same
    reason as the focus rule above: an `@layer` would lose to every unlayered
    rule in the host stylesheet.

    A class emitted here that is not in `PUBLISHED_COMPONENT_CLASSES` fails
    `test_no_component_class_is_published_without_its_contract`, so the
    stylesheet cannot grow a component the contract has not declared.
    """
    breakpoint_sm = token("breakpoint-sm").value
    breakpoint_lg = token("breakpoint-lg").value
    return "\n".join(
        (
            "/* Published components. Contracts: dotmac_ui.components.COMPONENTS */",
            ".dmui-empty-state {",
            f"{_INDENT}display: flex;",
            f"{_INDENT}flex-direction: column;",
            f"{_INDENT}align-items: center;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-3xl) "
            f"var({TOKEN_PREFIX}space-lg);",
            f"{_INDENT}text-align: center;",
            "}",
            "",
            ".dmui-empty-state__visual {",
            f"{_INDENT}display: inline-grid;",
            f"{_INDENT}width: var({TOKEN_PREFIX}space-3xl);",
            f"{_INDENT}height: var({TOKEN_PREFIX}space-3xl);",
            f"{_INDENT}place-items: center;",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-full);",
            f"{_INDENT}background: var({TOKEN_PREFIX}surface-secondary);",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-muted);",
            "}",
            "",
            ".dmui-empty-state__icon {",
            f"{_INDENT}width: var({TOKEN_PREFIX}space-xl);",
            f"{_INDENT}height: var({TOKEN_PREFIX}space-xl);",
            "}",
            "",
            ".dmui-empty-state__title {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-primary);",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-body);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-base);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-semibold);",
            f"{_INDENT}line-height: var({TOKEN_PREFIX}line-height-normal);",
            "}",
            "",
            ".dmui-empty-state__message {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-secondary);",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-body);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            f"{_INDENT}line-height: var({TOKEN_PREFIX}line-height-normal);",
            "}",
            "",
            ".dmui-empty-state__action {",
            f"{_INDENT}display: inline-flex;",
            f"{_INDENT}align-items: center;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-2xs);",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-xs) "
            f"var({TOKEN_PREFIX}space-md);",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-lg);",
            f"{_INDENT}background: var({TOKEN_PREFIX}action-primary-default);",
            f"{_INDENT}color: var({TOKEN_PREFIX}action-primary-on);",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-body);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-medium);",
            f"{_INDENT}text-decoration: none;",
            f"{_INDENT}transition: background var({TOKEN_PREFIX}duration-fast) "
            f"var({TOKEN_PREFIX}easing-standard);",
            "}",
            "",
            ".dmui-empty-state__action-icon {",
            f"{_INDENT}width: var({TOKEN_PREFIX}space-md);",
            f"{_INDENT}height: var({TOKEN_PREFIX}space-md);",
            "}",
            "",
            ".dmui-empty-state__action:hover {",
            f"{_INDENT}background: var({TOKEN_PREFIX}action-primary-hover);",
            "}",
            "",
            ".dmui-empty-state__action:active {",
            f"{_INDENT}background: var({TOKEN_PREFIX}action-primary-pressed);",
            "}",
            "",
            ".dmui-map-frame {",
            f"{_INDENT}position: relative;",
            f"{_INDENT}isolation: isolate;",
            f"{_INDENT}inline-size: 100%;",
            f"{_INDENT}min-block-size: var({TOKEN_PREFIX}map-frame-min-block-size);",
            f"{_INDENT}overflow: hidden;",
            f"{_INDENT}border: 1px solid var({TOKEN_PREFIX}border-subtle);",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-2xl);",
            f"{_INDENT}background: var({TOKEN_PREFIX}surface-secondary);",
            f"{_INDENT}box-shadow: var({TOKEN_PREFIX}shadow-sm);",
            "}",
            "",
            ".dmui-map-frame__canvas {",
            f"{_INDENT}inline-size: 100%;",
            f"{_INDENT}min-block-size: inherit;",
            f"{_INDENT}background: var({TOKEN_PREFIX}surface-secondary);",
            "}",
            "",
            ".dmui-map-frame__state {",
            f"{_INDENT}position: absolute;",
            f"{_INDENT}inset: 0;",
            f"{_INDENT}z-index: 2;",
            f"{_INDENT}display: none;",
            f"{_INDENT}place-items: center;",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-lg);",
            f"{_INDENT}background: var({TOKEN_PREFIX}surface-overlay);",
            "}",
            "",
            ".dmui-map-frame--ready .dmui-map-frame__state {",
            f"{_INDENT}display: none;",
            "}",
            "",
            ".dmui-map-frame--loading .dmui-map-frame__state,",
            ".dmui-map-frame--empty .dmui-map-frame__state,",
            ".dmui-map-frame--error .dmui-map-frame__state {",
            f"{_INDENT}display: grid;",
            "}",
            "",
            ".dmui-map-frame__state-panel {",
            f"{_INDENT}display: grid;",
            f"{_INDENT}max-inline-size: 32rem;",
            f"{_INDENT}justify-items: center;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-xs);",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-lg);",
            f"{_INDENT}border: 1px solid;",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-xl);",
            f"{_INDENT}box-shadow: var({TOKEN_PREFIX}shadow-md);",
            f"{_INDENT}text-align: center;",
            "}",
            "",
            ".dmui-map-frame__state-indicator {",
            f"{_INDENT}display: block;",
            f"{_INDENT}inline-size: var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}block-size: var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-full);",
            f"{_INDENT}background: currentColor;",
            "}",
            "",
            ".dmui-map-frame__state-title {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-body);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-base);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-semibold);",
            f"{_INDENT}line-height: var({TOKEN_PREFIX}line-height-normal);",
            "}",
            "",
            ".dmui-map-frame__state-message {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-body);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            f"{_INDENT}line-height: var({TOKEN_PREFIX}line-height-normal);",
            "}",
            "",
            ".dmui-map-frame--loading .dmui-map-frame__state-panel {",
            f"{_INDENT}border-color: var({TOKEN_PREFIX}status-info-border);",
            f"{_INDENT}background: var({TOKEN_PREFIX}status-info-surface);",
            f"{_INDENT}color: var({TOKEN_PREFIX}status-info-foreground);",
            "}",
            "",
            ".dmui-map-frame--empty .dmui-map-frame__state-panel {",
            f"{_INDENT}border-color: var({TOKEN_PREFIX}status-neutral-border);",
            f"{_INDENT}background: var({TOKEN_PREFIX}status-neutral-surface);",
            f"{_INDENT}color: var({TOKEN_PREFIX}status-neutral-foreground);",
            "}",
            "",
            ".dmui-map-frame--error .dmui-map-frame__state-panel {",
            f"{_INDENT}border-color: var({TOKEN_PREFIX}status-negative-border);",
            f"{_INDENT}background: var({TOKEN_PREFIX}status-negative-surface);",
            f"{_INDENT}color: var({TOKEN_PREFIX}status-negative-foreground);",
            "}",
            "",
            ".dmui-map-frame__live {",
            f"{_INDENT}position: absolute;",
            f"{_INDENT}inline-size: 1px;",
            f"{_INDENT}block-size: 1px;",
            f"{_INDENT}margin: -1px;",
            f"{_INDENT}overflow: hidden;",
            f"{_INDENT}clip: rect(0 0 0 0);",
            f"{_INDENT}white-space: nowrap;",
            "}",
            "",
            ".dmui-catalog-grid {",
            f"{_INDENT}display: grid;",
            f"{_INDENT}grid-template-columns: minmax(0, 1fr);",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-md);",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}padding: 0;",
            f"{_INDENT}list-style: none;",
            "}",
            "",
            f"@media (min-width: {breakpoint_sm}) {{",
            f"{_INDENT}.dmui-catalog-grid {{",
            f"{_INDENT * 2}grid-template-columns: repeat(2, minmax(0, 1fr));",
            f"{_INDENT}}}",
            "}",
            "",
            f"@media (min-width: {breakpoint_lg}) {{",
            f"{_INDENT}.dmui-catalog-grid {{",
            f"{_INDENT * 2}grid-template-columns: repeat(3, minmax(0, 1fr));",
            f"{_INDENT}}}",
            "}",
            "",
            ".dmui-catalog-grid__item {",
            f"{_INDENT}display: flex;",
            f"{_INDENT}min-width: 0;",
            f"{_INDENT}flex-direction: column;",
            f"{_INDENT}overflow: hidden;",
            f"{_INDENT}border: 1px solid var({TOKEN_PREFIX}border-default);",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-xl);",
            f"{_INDENT}background: var({TOKEN_PREFIX}surface-elevated);",
            f"{_INDENT}box-shadow: var({TOKEN_PREFIX}shadow-sm);",
            "}",
            "",
            ".dmui-catalog-grid__media {",
            f"{_INDENT}display: block;",
            f"{_INDENT}width: 100%;",
            f"{_INDENT}aspect-ratio: 16 / 9;",
            f"{_INDENT}border-bottom: 1px solid var({TOKEN_PREFIX}border-subtle);",
            f"{_INDENT}background: var({TOKEN_PREFIX}surface-secondary);",
            f"{_INDENT}object-fit: cover;",
            "}",
            "",
            ".dmui-catalog-grid__body {",
            f"{_INDENT}display: flex;",
            f"{_INDENT}flex: 1;",
            f"{_INDENT}flex-direction: column;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-xs);",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-md);",
            "}",
            "",
            ".dmui-catalog-grid__title {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-primary);",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-display);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-lg);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-semibold);",
            f"{_INDENT}line-height: var({TOKEN_PREFIX}line-height-snug);",
            "}",
            "",
            ".dmui-catalog-grid__meta {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-tertiary);",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-body);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-medium);",
            f"{_INDENT}line-height: var({TOKEN_PREFIX}line-height-normal);",
            "}",
            "",
            ".dmui-catalog-grid__description {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-secondary);",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-body);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            f"{_INDENT}line-height: var({TOKEN_PREFIX}line-height-normal);",
            "}",
            "",
            ".dmui-catalog-grid__notice {",
            f"{_INDENT}margin: var({TOKEN_PREFIX}space-2xs) 0 0;",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-xs) "
            f"var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}border-left: var({TOKEN_PREFIX}focus-ring-width) solid "
            f"var({TOKEN_PREFIX}status-warning-indicator);",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-sm);",
            f"{_INDENT}background: var({TOKEN_PREFIX}status-warning-surface);",
            f"{_INDENT}color: var({TOKEN_PREFIX}status-warning-foreground);",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-body);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            f"{_INDENT}line-height: var({TOKEN_PREFIX}line-height-normal);",
            "}",
            "",
            ".dmui-catalog-grid__action {",
            f"{_INDENT}display: inline-flex;",
            f"{_INDENT}width: fit-content;",
            f"{_INDENT}align-items: center;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-2xs);",
            f"{_INDENT}margin-top: auto;",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-xs) "
            f"var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-lg);",
            f"{_INDENT}background: var({TOKEN_PREFIX}action-primary-default);",
            f"{_INDENT}color: var({TOKEN_PREFIX}action-primary-on);",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-body);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-medium);",
            f"{_INDENT}text-decoration: none;",
            f"{_INDENT}transition: background var({TOKEN_PREFIX}duration-fast) "
            f"var({TOKEN_PREFIX}easing-standard);",
            "}",
            "",
            ".dmui-catalog-grid__action-icon {",
            f"{_INDENT}width: var({TOKEN_PREFIX}space-md);",
            f"{_INDENT}height: var({TOKEN_PREFIX}space-md);",
            "}",
            "",
            ".dmui-catalog-grid__action:hover {",
            f"{_INDENT}background: var({TOKEN_PREFIX}action-primary-hover);",
            "}",
            "",
            ".dmui-catalog-grid__action:active {",
            f"{_INDENT}background: var({TOKEN_PREFIX}action-primary-pressed);",
            "}",
        )
    )


def _list_and_activity_block() -> str:
    """Token-native list/queue and recent-activity component styles."""
    breakpoint_sm = token("breakpoint-sm").value
    return "\n".join(
        (
            ".dmui-list-surface,",
            ".dmui-recent-activity {",
            f"{_INDENT}border: 1px solid var({TOKEN_PREFIX}border-default);",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-xl);",
            f"{_INDENT}background: var({TOKEN_PREFIX}surface-elevated);",
            f"{_INDENT}box-shadow: var({TOKEN_PREFIX}shadow-sm);",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-primary);",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-body);",
            "}",
            "",
            ".dmui-list-surface__title {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-md) "
            f"var({TOKEN_PREFIX}space-lg);",
            f"{_INDENT}border-bottom: 1px solid var({TOKEN_PREFIX}border-subtle);",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-display);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-lg);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-semibold);",
            "}",
            "",
            ".dmui-list-surface__controls {",
            f"{_INDENT}display: flex;",
            f"{_INDENT}flex-wrap: wrap;",
            f"{_INDENT}align-items: end;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-md) "
            f"var({TOKEN_PREFIX}space-lg);",
            f"{_INDENT}border-bottom: 1px solid var({TOKEN_PREFIX}border-subtle);",
            "}",
            "",
            ".dmui-list-surface__search,",
            ".dmui-list-surface__filter {",
            f"{_INDENT}display: grid;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-2xs);",
            "}",
            "",
            ".dmui-list-surface__search {",
            f"{_INDENT}min-inline-size: min(100%, 18rem);",
            f"{_INDENT}flex: 1;",
            "}",
            "",
            ".dmui-list-surface__filters {",
            f"{_INDENT}display: flex;",
            f"{_INDENT}flex-wrap: wrap;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-sm);",
            "}",
            "",
            ".dmui-list-surface__search-label,",
            ".dmui-list-surface__filter-label {",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-secondary);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-xs);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-medium);",
            "}",
            "",
            ".dmui-list-surface__search-input,",
            ".dmui-list-surface__select {",
            f"{_INDENT}min-block-size: var({TOKEN_PREFIX}control-min-block-size);",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-xs) "
            f"var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}border: 1px solid var({TOKEN_PREFIX}border-default);",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-md);",
            f"{_INDENT}background: var({TOKEN_PREFIX}surface-primary);",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-primary);",
            f"{_INDENT}font: inherit;",
            "}",
            "",
            ".dmui-list-surface__apply,",
            ".dmui-list-surface__row-action {",
            f"{_INDENT}display: inline-flex;",
            f"{_INDENT}min-block-size: var({TOKEN_PREFIX}control-min-block-size);",
            f"{_INDENT}align-items: center;",
            f"{_INDENT}justify-content: center;",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-xs) "
            f"var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}border: 0;",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-md);",
            f"{_INDENT}background: var({TOKEN_PREFIX}action-primary-default);",
            f"{_INDENT}color: var({TOKEN_PREFIX}action-primary-on);",
            f"{_INDENT}font: inherit;",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-medium);",
            f"{_INDENT}text-decoration: none;",
            f"{_INDENT}cursor: pointer;",
            "}",
            "",
            ".dmui-list-surface__apply:hover,",
            ".dmui-list-surface__row-action:hover {",
            f"{_INDENT}background: var({TOKEN_PREFIX}action-primary-hover);",
            "}",
            "",
            ".dmui-list-surface__table-wrap {",
            f"{_INDENT}overflow-x: auto;",
            "}",
            "",
            ".dmui-list-surface__table {",
            f"{_INDENT}width: 100%;",
            f"{_INDENT}border-collapse: collapse;",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            "}",
            "",
            ".dmui-list-surface__head {",
            f"{_INDENT}background: var({TOKEN_PREFIX}surface-secondary);",
            "}",
            "",
            ".dmui-list-surface__heading,",
            ".dmui-list-surface__cell {",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-sm) "
            f"var({TOKEN_PREFIX}space-md);",
            f"{_INDENT}text-align: left;",
            f"{_INDENT}vertical-align: middle;",
            "}",
            "",
            ".dmui-list-surface__heading {",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-secondary);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-xs);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-semibold);",
            f"{_INDENT}letter-spacing: var({TOKEN_PREFIX}letter-spacing-wide);",
            f"{_INDENT}text-transform: uppercase;",
            "}",
            "",
            ".dmui-list-surface__sort {",
            f"{_INDENT}display: inline-flex;",
            f"{_INDENT}align-items: center;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-2xs);",
            f"{_INDENT}color: inherit;",
            f"{_INDENT}text-decoration: none;",
            "}",
            "",
            ".dmui-list-surface__sort-indicator {",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-primary);",
            "}",
            "",
            ".dmui-list-surface__row {",
            f"{_INDENT}border-top: 1px solid var({TOKEN_PREFIX}border-subtle);",
            "}",
            "",
            ".dmui-list-surface__cell-link {",
            f"{_INDENT}display: grid;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-3xs);",
            f"{_INDENT}color: inherit;",
            f"{_INDENT}text-decoration: none;",
            "}",
            "",
            ".dmui-list-surface__cell-text {",
            f"{_INDENT}display: block;",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-primary);",
            "}",
            "",
            ".dmui-list-surface__cell-secondary {",
            f"{_INDENT}display: block;",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-tertiary);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-xs);",
            "}",
            "",
            ".dmui-list-surface__state {",
            f"{_INDENT}display: inline-flex;",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-3xs) "
            f"var({TOKEN_PREFIX}space-xs);",
            f"{_INDENT}border: 1px solid var({TOKEN_PREFIX}status-neutral-border);",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-full);",
            f"{_INDENT}background: var({TOKEN_PREFIX}status-neutral-surface);",
            f"{_INDENT}color: var({TOKEN_PREFIX}status-neutral-foreground);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-medium);",
            "}",
            "",
            '.dmui-list-surface__state[data-dmui-tone="positive"] {',
            f"{_INDENT}border-color: var({TOKEN_PREFIX}status-positive-border);",
            f"{_INDENT}background: var({TOKEN_PREFIX}status-positive-surface);",
            f"{_INDENT}color: var({TOKEN_PREFIX}status-positive-foreground);",
            "}",
            "",
            '.dmui-list-surface__state[data-dmui-tone="info"] {',
            f"{_INDENT}border-color: var({TOKEN_PREFIX}status-info-border);",
            f"{_INDENT}background: var({TOKEN_PREFIX}status-info-surface);",
            f"{_INDENT}color: var({TOKEN_PREFIX}status-info-foreground);",
            "}",
            "",
            '.dmui-list-surface__state[data-dmui-tone="warning"] {',
            f"{_INDENT}border-color: var({TOKEN_PREFIX}status-warning-border);",
            f"{_INDENT}background: var({TOKEN_PREFIX}status-warning-surface);",
            f"{_INDENT}color: var({TOKEN_PREFIX}status-warning-foreground);",
            "}",
            "",
            '.dmui-list-surface__state[data-dmui-tone="negative"] {',
            f"{_INDENT}border-color: var({TOKEN_PREFIX}status-negative-border);",
            f"{_INDENT}background: var({TOKEN_PREFIX}status-negative-surface);",
            f"{_INDENT}color: var({TOKEN_PREFIX}status-negative-foreground);",
            "}",
            "",
            ".dmui-list-surface__actions {",
            f"{_INDENT}inline-size: 1%;",
            f"{_INDENT}white-space: nowrap;",
            "}",
            "",
            ".dmui-list-surface__footer {",
            f"{_INDENT}display: flex;",
            f"{_INDENT}flex-wrap: wrap;",
            f"{_INDENT}align-items: center;",
            f"{_INDENT}justify-content: space-between;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-md) "
            f"var({TOKEN_PREFIX}space-lg);",
            f"{_INDENT}border-top: 1px solid var({TOKEN_PREFIX}border-subtle);",
            "}",
            "",
            ".dmui-list-surface__summary {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-secondary);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            "}",
            "",
            ".dmui-list-surface__pagination {",
            f"{_INDENT}display: flex;",
            f"{_INDENT}align-items: center;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-2xs);",
            "}",
            "",
            ".dmui-list-surface__page {",
            f"{_INDENT}display: inline-grid;",
            f"{_INDENT}min-inline-size: var({TOKEN_PREFIX}control-min-block-size);",
            f"{_INDENT}min-block-size: var({TOKEN_PREFIX}control-min-block-size);",
            f"{_INDENT}place-items: center;",
            f"{_INDENT}padding: 0 var({TOKEN_PREFIX}space-xs);",
            f"{_INDENT}border: 1px solid var({TOKEN_PREFIX}border-default);",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-md);",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-secondary);",
            f"{_INDENT}text-decoration: none;",
            "}",
            "",
            ".dmui-list-surface__page--current {",
            f"{_INDENT}border-color: var({TOKEN_PREFIX}action-primary-default);",
            f"{_INDENT}background: var({TOKEN_PREFIX}action-primary-default);",
            f"{_INDENT}color: var({TOKEN_PREFIX}action-primary-on);",
            "}",
            "",
            ".dmui-list-surface__page--disabled {",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-muted);",
            f"{_INDENT}cursor: not-allowed;",
            "}",
            "",
            ".dmui-list-surface__ellipsis {",
            f"{_INDENT}padding: 0 var({TOKEN_PREFIX}space-2xs);",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-tertiary);",
            "}",
            "",
            ".dmui-list-surface__heading--desktop,",
            ".dmui-list-surface__cell--desktop {",
            f"{_INDENT}display: none;",
            "}",
            "",
            f"@media (min-width: {breakpoint_sm}) {{",
            f"{_INDENT}.dmui-list-surface__heading--desktop,",
            f"{_INDENT}.dmui-list-surface__cell--desktop {{",
            f"{_INDENT * 2}display: table-cell;",
            f"{_INDENT}}}",
            "}",
            "",
            ".dmui-recent-activity__header {",
            f"{_INDENT}display: flex;",
            f"{_INDENT}align-items: center;",
            f"{_INDENT}justify-content: space-between;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-md) "
            f"var({TOKEN_PREFIX}space-lg);",
            f"{_INDENT}border-bottom: 1px solid var({TOKEN_PREFIX}border-subtle);",
            "}",
            "",
            ".dmui-recent-activity__heading {",
            f"{_INDENT}display: flex;",
            f"{_INDENT}align-items: center;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-xs);",
            "}",
            "",
            ".dmui-recent-activity__heading-icon,",
            ".dmui-recent-activity__marker,",
            ".dmui-recent-activity__empty-icon {",
            f"{_INDENT}display: inline-grid;",
            f"{_INDENT}place-items: center;",
            f"{_INDENT}border-radius: var({TOKEN_PREFIX}radius-lg);",
            f"{_INDENT}background: var({TOKEN_PREFIX}surface-secondary);",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-tertiary);",
            "}",
            "",
            ".dmui-recent-activity__heading-icon,",
            ".dmui-recent-activity__marker {",
            f"{_INDENT}inline-size: var({TOKEN_PREFIX}space-xl);",
            f"{_INDENT}block-size: var({TOKEN_PREFIX}space-xl);",
            "}",
            "",
            ".dmui-recent-activity__title {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}font-family: var({TOKEN_PREFIX}font-display);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-base);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-semibold);",
            "}",
            "",
            ".dmui-recent-activity__all-link {",
            f"{_INDENT}color: var({TOKEN_PREFIX}action-primary-default);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-medium);",
            "}",
            "",
            ".dmui-recent-activity__list {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}padding: 0;",
            f"{_INDENT}list-style: none;",
            "}",
            "",
            ".dmui-recent-activity__item {",
            f"{_INDENT}border-top: 1px solid var({TOKEN_PREFIX}border-subtle);",
            "}",
            "",
            ".dmui-recent-activity__item:first-child {",
            f"{_INDENT}border-top: 0;",
            "}",
            "",
            ".dmui-recent-activity__item-link {",
            f"{_INDENT}display: flex;",
            f"{_INDENT}align-items: start;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-sm);",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-sm) "
            f"var({TOKEN_PREFIX}space-lg);",
            f"{_INDENT}color: inherit;",
            f"{_INDENT}text-decoration: none;",
            "}",
            "",
            ".dmui-recent-activity__body {",
            f"{_INDENT}display: grid;",
            f"{_INDENT}min-inline-size: 0;",
            f"{_INDENT}flex: 1;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-3xs);",
            "}",
            "",
            ".dmui-recent-activity__message {",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-primary);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-medium);",
            "}",
            "",
            ".dmui-recent-activity__detail,",
            ".dmui-recent-activity__time {",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-tertiary);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-xs);",
            "}",
            "",
            ".dmui-recent-activity__time {",
            f"{_INDENT}white-space: nowrap;",
            "}",
            "",
            ".dmui-recent-activity__empty {",
            f"{_INDENT}display: grid;",
            f"{_INDENT}justify-items: center;",
            f"{_INDENT}gap: var({TOKEN_PREFIX}space-xs);",
            f"{_INDENT}padding: var({TOKEN_PREFIX}space-3xl) "
            f"var({TOKEN_PREFIX}space-lg);",
            f"{_INDENT}text-align: center;",
            "}",
            "",
            ".dmui-recent-activity__empty-icon {",
            f"{_INDENT}inline-size: var({TOKEN_PREFIX}space-3xl);",
            f"{_INDENT}block-size: var({TOKEN_PREFIX}space-3xl);",
            "}",
            "",
            ".dmui-recent-activity__empty-title {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}font-weight: var({TOKEN_PREFIX}font-weight-medium);",
            "}",
            "",
            ".dmui-recent-activity__empty-message {",
            f"{_INDENT}margin: 0;",
            f"{_INDENT}color: var({TOKEN_PREFIX}text-tertiary);",
            f"{_INDENT}font-size: var({TOKEN_PREFIX}font-size-sm);",
            "}",
        )
    )


def render_stylesheet(package_version: str) -> str:
    """The complete compiled stylesheet. Deterministic."""
    sections = (
        _banner(package_version),
        _root_block(),
        _dark_block(),
        _reduced_motion_block(),
        _base_block(),
        _components_block(),
        _list_and_activity_block(),
    )
    return "\n\n".join(sections) + "\n"


#: Token category -> the Tailwind theme key it feeds, and the prefix to strip
#: from each token name. Two categories are renamed on the way through:
#: `text-*` would produce `text-text-primary` and `border-*` would produce
#: `border-border-subtle`, because Tailwind derives utility names from the
#: colour group. `content` and `stroke` keep the generated utilities readable.
_COLOR_GROUPS: tuple[tuple[str, str], ...] = (
    ("color", "color-"),
    ("surface", "surface-"),
    ("text", "text-"),
    ("border", "border-"),
    ("action", "action-"),
    ("status", "status-"),
)
_COLOR_GROUP_NAMES = {"text": "content", "border": "stroke", "color": ""}


def _nest(flat: dict[str, str]) -> dict[str, object]:
    """`{"brand-500": v}` -> `{"brand": {"500": v}}`, one level deep.

    Tailwind reads nested groups as `bg-brand-500`; a flat key with a dash works
    too, but nesting is what makes a palette legible in the generated file.
    """
    out: dict[str, object] = {}
    for key, value in flat.items():
        head, _, tail = key.partition("-")
        if tail:
            bucket = out.setdefault(head, {})
            if isinstance(bucket, dict):
                bucket[tail] = value
            continue
        out[key] = value
    return out


def _colors() -> dict[str, object]:
    colors: dict[str, object] = {}
    for category, prefix in _COLOR_GROUPS:
        flat = {
            design_token.name[len(prefix) :]: (
                f"rgb(var({channel_variable(design_token.name)}) / <alpha-value>)"
            )
            for design_token in tokens_in(category)
        }
        group = _COLOR_GROUP_NAMES.get(category, category)
        if group:
            colors[group] = _nest(flat)
        else:
            colors.update(_nest(flat))
    return colors


def _scale(category: str, prefix: str) -> dict[str, str]:
    return {
        design_token.name[len(prefix) :]: f"var({design_token.variable})"
        for design_token in tokens_in(category)
    }


def _literal_scale(category: str, prefix: str) -> dict[str, str]:
    """Resolved values, for places a CSS variable cannot be used.

    Media queries are the case that matters: `@media (min-width: var(--x))` is
    not valid CSS — custom properties are not substituted in a media condition.
    A preset that emitted `var()` for `screens` would compile without complaint
    and silently break every responsive utility, which is the worst shape a bug
    can take here.

    The cost is that breakpoints are baked at build time and a brand override
    cannot move them. That is the correct trade: breakpoints are a layout
    contract, not a brand decision.
    """
    return {
        design_token.name[len(prefix) :]: design_token.value
        for design_token in tokens_in(category)
    }


def render_tailwind_preset(package_version: str) -> str:
    """A Tailwind preset pointing every utility at a token variable.

    Generated rather than written, for the same reason the stylesheet is: a
    hand-maintained preset is a second copy of the token names, and a second
    copy drifts. `dotmac_erp` and `dotmac_sub` have two copies of
    `base/_tokens.css` that are now 48% divergent.

    Utilities resolve to `var(--dmui-*)`, so a consumer's compiled CSS contains
    variable references, not values — one stylesheet swap re-themes it, and dark
    mode needs no `dark:` variant on any element.

    Colour utilities use the published channel variables, so opacity modifiers
    (`bg-brand-500/50`) compile to ``rgb(var(--dmui-...-rgb) / 0.5)``. Runtime
    brand projection therefore updates both ramp channels and every semantic
    role channel that resolves through them; a whole-colour override alone would
    split solid and translucent uses across two palettes.
    """
    preset = {
        "darkMode": ["selector", DARK_THEME_SELECTORS[-1]],
        "theme": {
            "extend": {
                "colors": _colors(),
                "spacing": _scale("space", "space-"),
                "borderRadius": _scale("radius", "radius-"),
                "boxShadow": _scale("shadow", "shadow-"),
                "screens": _literal_scale("breakpoint", "breakpoint-"),
            }
        },
    }
    body = json.dumps(preset, indent=2, sort_keys=False)
    return (
        f"// Generated by dotmac-ui {package_version}. Do not edit.\n"
        f"// UI contract version {UI_CONTRACT_VERSION}.\n"
        "//\n"
        "// Usage, in a consumer's tailwind.config.js:\n"
        "//   const preset = require('<dotmac_ui>/static/"
        f"{TAILWIND_PRESET_RELPATH}');\n"
        "//   module.exports = { presets: [preset], content: [...] };\n"
        "//\n"
        "// Resolve <dotmac_ui> from the installed package rather than copying\n"
        '// this file: `python -c "import dotmac_ui.assets as a;'
        ' print(a.tailwind_preset_path())"`.\n'
        f"module.exports = {body};\n"
    )


def render_behavior_script(package_version: str) -> str:
    """Publish the reviewed JS source with deterministic contract metadata."""
    source = _BEHAVIOR_SOURCE_PATH.read_text(encoding="utf-8")
    return (
        f"// Generated by dotmac-ui {package_version}. Do not edit.\n"
        f"// UI contract version {UI_CONTRACT_VERSION}.\n" + source
    )


def render_manifest(
    package_version: str,
    stylesheet: str,
    behavior_script: str | None = None,
) -> str:
    """The published asset manifest. Deterministic; ends with a newline."""
    script = behavior_script or render_behavior_script(package_version)
    payload = {
        "name": "dotmac-ui",
        "version": package_version,
        "ui_contract_version": UI_CONTRACT_VERSION,
        "token_prefix": TOKEN_PREFIX,
        "token_count": len(TOKENS),
        "assets": [
            {
                "path": STYLESHEET_RELPATH,
                "bytes": len(stylesheet.encode("utf-8")),
                "sha256": sha256(stylesheet.encode("utf-8")).hexdigest(),
            },
            {
                "path": BEHAVIOR_SCRIPT_RELPATH,
                "bytes": len(script.encode("utf-8")),
                "sha256": sha256(script.encode("utf-8")).hexdigest(),
            },
        ],
    }
    return json.dumps(payload, indent=2, sort_keys=True) + "\n"


def rendered_assets(package_version: str) -> dict[str, str]:
    """`{relative path: contents}` for every published asset."""
    stylesheet = render_stylesheet(package_version)
    behavior_script = render_behavior_script(package_version)
    return {
        STYLESHEET_RELPATH: stylesheet,
        BEHAVIOR_SCRIPT_RELPATH: behavior_script,
        TAILWIND_PRESET_RELPATH: render_tailwind_preset(package_version),
        MANIFEST_RELPATH: render_manifest(
            package_version,
            stylesheet,
            behavior_script,
        ),
    }


def write_assets(package_version: str) -> list[Path]:
    """Write every published asset into the package's static directory."""
    written: list[Path] = []
    for relpath, contents in rendered_assets(package_version).items():
        target = static_dir() / relpath
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(contents, encoding="utf-8")
        written.append(target)
    return written


def main(argv: list[str] | None = None) -> int:
    from dotmac_ui import __version__

    parser = argparse.ArgumentParser(description="Build the dotmac-ui assets.")
    parser.add_argument(
        "--check",
        action="store_true",
        help="Fail (exit 1) if the committed assets differ from a fresh build "
        "instead of rewriting them.",
    )
    args = parser.parse_args(argv)

    expected = rendered_assets(__version__)
    if args.check:
        stale = [
            relpath
            for relpath, contents in expected.items()
            if not (static_dir() / relpath).is_file()
            or (static_dir() / relpath).read_text(encoding="utf-8") != contents
        ]
        if stale:
            print(
                "dotmac-ui assets are stale; run `make ui-build`:\n  "
                + "\n  ".join(stale),
                file=sys.stderr,
            )
            return 1
        print(f"dotmac-ui assets up to date ({len(expected)} files).")
        return 0

    for path in write_assets(__version__):
        print(f"wrote {path}")
    return 0


if __name__ == "__main__":  # pragma: no cover - CLI entry
    raise SystemExit(main())
