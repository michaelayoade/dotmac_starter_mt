...
Ported from dotmac_sub:app/services/unit_of_work.py (org infrastructure
source of truth); `get_uow` is ported from dotmac_sub:app/db.py (that repo
splits the dependency out of the UnitOfWork module — this port keeps it in
one file since app/core/unit_of_work.py is this task's whole surface area).
    ...
    Example:
        ...
        def create_item(
            ...
            with uow:
                ...
                return item
    """
    # Imported lazily: app.core.db creates its engine at import time from
    # settings.database_url, which is unset in the unit-test environment
    # (SQLite fixtures in tests/unit/conftest.py don't go through it). A
    # module-level import here would break `import app.core.unit_of_work`
    # under pytest before get_uow is ever called.
